  ...
  it("keeps intake, archive verification, and decision-record routes registered through the shared route table", async () => {
    ...
    try {
      ...
      const json = await app.inject({
        method: "GET",
        url: LATEST_MANAGED_AUDIT_DISABLED_READ_ONLY_INTEGRATION_ROUTE,
        headers: completeHeaders(),
      });
      const markdown = await app.inject({
        method: "GET",
        url: `${LATEST_MANAGED_AUDIT_DISABLED_READ_ONLY_INTEGRATION_ROUTE}?format=markdown`,
        headers: completeHeaders(),
      });
      ...
      expect(markdown.body).toContain("managed-audit-disabled read-only integration decision record");
...
function completeHeaders() {
  return {
    "x-orderops-operator-id": "auditor-416",
    "x-orderops-roles": "admin,auditor,operator,viewer",
    "x-orderops-operator-verified": "true",
    "x-orderops-approval-correlation-id": "approval-v416-route-split",
  };
}
