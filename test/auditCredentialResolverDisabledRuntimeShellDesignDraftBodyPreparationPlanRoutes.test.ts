...
} from "../src/routes/auditCredentialResolverDisabledRuntimeShellDesignDraftBodyPreparationPlanRoutes.js";

import { expectAuditRouteGroupRegisteredThroughCatalog } from "./support/auditJsonMarkdownRouteCatalogTestSupport.js";

const LATEST_DISABLED_RUNTIME_SHELL_DESIGN_DRAFT_BODY_PREPARATION_PLAN_ROUTE =
...
describe("credential resolver disabled runtime shell design draft body preparation plan audit route group", () => {
  it("keeps design draft body preparation plan routes registered through the shared route table", async () => {
    const previous = process.env[FORCE_FALLBACK_ENV];
    process.env[FORCE_FALLBACK_ENV] = "true";
    const app = await buildApp(loadTestConfig());
    try {
      const paths = credentialResolverDisabledRuntimeShellDesignDraftBodyPreparationPlanAuditJsonMarkdownRoutes.map((route) => route.path);
      const json = await app.inject({
        method: "GET",
        url: LATEST_DISABLED_RUNTIME_SHELL_DESIGN_DRAFT_BODY_PREPARATION_PLAN_ROUTE,
        headers: completeHeaders(),
      });
      const markdown = await app.inject({
        method: "GET",
        url: `${LATEST_DISABLED_RUNTIME_SHELL_DESIGN_DRAFT_BODY_PREPARATION_PLAN_ROUTE}?format=markdown`,
        headers: completeHeaders(),
      });
      ...
      expect(paths).toContain(LATEST_DISABLED_RUNTIME_SHELL_DESIGN_DRAFT_BODY_PREPARATION_PLAN_ROUTE);
      expectAuditRouteGroupRegisteredThroughCatalog({
        routes: credentialResolverDisabledRuntimeShellDesignDraftBodyPreparationPlanAuditJsonMarkdownRoutes,
        sourceAnchor: "...credentialResolverDisabledRuntimeShellDesignDraftBodyPreparationPlanAuditJsonMarkdownRoutes",
      });
      ...
      expect(markdown.body).toContain("disabled runtime shell design draft body preparation plan archive verification");
...
function completeHeaders() {
  return {
    "x-orderops-operator-id": "auditor-441",
    "x-orderops-roles": "auditor,operator,viewer",
    "x-orderops-operator-verified": "true",
    "x-orderops-approval-correlation-id": "approval-v441-route-split",
  };
}
