...
describe("credential resolver final prerequisite audit route group", () => {
  it("keeps abort rollback readiness and final prerequisite routes registered through the shared route table", async () => {
    ...
    try {
      ...
      const json = await app.inject({
        method: "GET",
        url: LATEST_FINAL_PREREQUISITE_ROUTE,
        headers: completeHeaders(),
      });
      const markdown = await app.inject({
        method: "GET",
        url: `${LATEST_FINAL_PREREQUISITE_ROUTE}?format=markdown`,
        headers: completeHeaders(),
      });
      ...
      expect(markdown.body).toContain("final prerequisite closure review");
...
function completeHeaders() {
  return {
    "x-orderops-operator-id": "auditor-434",
    "x-orderops-roles": "auditor,operator,viewer",
    "x-orderops-operator-verified": "true",
    "x-orderops-approval-correlation-id": "approval-v434-route-split",
  };
}
