...
describe("controlled read-only shard preview type module catalog", () => {
  it("records stable type module ownership groups", () => {
    ...
    expect(catalog.catalogVersion).toBe("Node v1456");
    ...
    expect(catalog.moduleCount).toBe(209);
  ...
  it("renders a stable markdown catalog for archive review", () => {
    ...
    expect(markdown).toContain("- Catalog version: Node v1456");
    expect(markdown).toContain("- Module count: 209");
    ...
    expect(markdown).toContain("### 209. profile-entry-types");
