...
describe("credential resolver endpoint handle allowlist approval audit route group", () => {
  it("keeps endpoint handle allowlist approval routes registered through the shared route table", async () => {
    ...
    try {
      ...
      const json = await app.inject({
        method: "GET",
        url: LATEST_ENDPOINT_HANDLE_ALLOWLIST_APPROVAL_ROUTE,
        headers: completeHeaders(),
      });
      const markdown = await app.inject({
        method: "GET",
        url: `${LATEST_ENDPOINT_HANDLE_ALLOWLIST_APPROVAL_ROUTE}?format=markdown`,
        headers: completeHeaders(),
      });
      ...
      expect(markdown.body).toContain("endpoint handle allowlist approval prerequisite closure review");
...
function completeHeaders() {
  return {
    "x-orderops-operator-id": "auditor-432",
    "x-orderops-roles": "auditor,operator,viewer",
    "x-orderops-operator-verified": "true",
    "x-orderops-approval-correlation-id": "approval-v432-route-split",
  };
}
