...
} from "../src/routes/auditCredentialResolverDisabledRuntimeShellDesignDraftOutlineRoutes.js";

import { expectAuditRouteGroupRegisteredThroughCatalog } from "./support/auditJsonMarkdownRouteCatalogTestSupport.js";

const LATEST_DISABLED_RUNTIME_SHELL_DESIGN_DRAFT_OUTLINE_ROUTE =
...
describe("credential resolver disabled runtime shell design draft outline audit route group", () => {
  it("keeps design draft outline routes registered through the shared route table", async () => {
    const previous = process.env[FORCE_FALLBACK_ENV];
    process.env[FORCE_FALLBACK_ENV] = "true";
    const app = await buildApp(loadTestConfig());
    try {
      const paths = credentialResolverDisabledRuntimeShellDesignDraftOutlineAuditJsonMarkdownRoutes.map((route) => route.path);
      const json = await app.inject({
        method: "GET",
        url: LATEST_DISABLED_RUNTIME_SHELL_DESIGN_DRAFT_OUTLINE_ROUTE,
        headers: completeHeaders(),
      });
      const markdown = await app.inject({
        method: "GET",
        url: `${LATEST_DISABLED_RUNTIME_SHELL_DESIGN_DRAFT_OUTLINE_ROUTE}?format=markdown`,
        headers: completeHeaders(),
      });
      ...
      expect(paths).toContain(LATEST_DISABLED_RUNTIME_SHELL_DESIGN_DRAFT_OUTLINE_ROUTE);
      expectAuditRouteGroupRegisteredThroughCatalog({
        routes: credentialResolverDisabledRuntimeShellDesignDraftOutlineAuditJsonMarkdownRoutes,
        sourceAnchor: "...credentialResolverDisabledRuntimeShellDesignDraftOutlineAuditJsonMarkdownRoutes",
      });
      ...
      expect(markdown.body).toContain("disabled runtime shell design draft outline archive verification");
...
function completeHeaders() {
  return {
    "x-orderops-operator-id": "auditor-437",
    "x-orderops-roles": "auditor,operator,viewer",
    "x-orderops-operator-verified": "true",
    "x-orderops-approval-correlation-id": "approval-v437-route-split",
  };
}
