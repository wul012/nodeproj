# Code walkthrough 1122: Node v1117 operator role review lane

This walkthrough explains how Node v1117 participates in the signed approval artifact draft readiness package. The implementation is intentionally split so the long governance chain stays maintainable instead of turning the profile service into one giant file.
...
- v1117 lane: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_READINESS_OPERATOR_ROLE_REVIEW`.
- Source field checked by builder: `operatorRole`.
- Blocker emitted by control catalog: `REJECT_DRAFT_READINESS_OPERATOR_ROLE_UNREVIEWED`.
