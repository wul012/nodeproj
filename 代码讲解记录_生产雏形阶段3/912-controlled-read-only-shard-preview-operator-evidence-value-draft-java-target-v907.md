# Node v907 code walkthrough: operator evidence value draft Java target

v907 contributes `VALUE_DRAFT_JAVA_TARGET` to the operator evidence value draft package. It maps back to `IMPORT_PREFLIGHT_JAVA_TARGET` and adds Java target status draft labels without accepting actual values.
