# Node v889 code walkthrough: operator evidence value draft version order

v889 contributes `VALUE_DRAFT_VERSION_ORDER` to the operator evidence value draft package. It maps back to `IMPORT_PREFLIGHT_VERSION_ORDER` and adds canonical version order draft labels without accepting actual values.
