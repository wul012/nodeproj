# Node v858 code walkthrough: manual evidence entry worksheet mini-kv target

v858 contributes `ENTRY_WORKSHEET_MINI_KV_TARGET` to the blank manual evidence entry worksheet. It maps back to `INTAKE_REVIEW_MINI_KV_TARGET` from the v836 review package and adds the mini-kv SHARDJSON status fields while keeping the command allowlist narrow.
