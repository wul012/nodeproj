# Code walkthrough 1118: Node v1113 artifact preflight digest pin lane

This walkthrough explains how Node v1113 participates in the signed approval artifact draft readiness package. The implementation is intentionally split so the long governance chain stays maintainable instead of turning the profile service into one giant file.
...
- v1113 lane: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_READINESS_ARTIFACT_PREFLIGHT_DIGEST_PIN`.
- Source field checked by builder: `sourceSignedApprovalCaptureArtifactPreflightDigest`.
- Blocker emitted by control catalog: `REJECT_DRAFT_READINESS_ARTIFACT_PREFLIGHT_DIGEST_UNPINNED`.
