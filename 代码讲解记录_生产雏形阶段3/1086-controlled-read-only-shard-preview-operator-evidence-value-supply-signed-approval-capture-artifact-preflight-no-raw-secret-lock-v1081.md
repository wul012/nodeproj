# Code walkthrough: Node v1081 signed approval capture artifact preflight no raw secret artifact lock
...
Node v1081 contributes no raw secret artifact lock. The code keeps readyForSignedApprovalCapture=false, readyForOperatorValueSupply=false, readyForEvidenceImport=false, readyForRuntimePayload=false, and executionAllowed=false.

## Version-Specific Mapping

- Fragment: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_PREFLIGHT_NO_RAW_SECRET_LOCK`.
- Seal: `ARTIFACT_PREFLIGHT_NO_RAW_SECRET_SEAL`.
- Source capture input: `SIGNED_APPROVAL_CAPTURE_PREFLIGHT_NO_RAW_SECRET_LOCK`.
- Required capture field: `noRawSecretLock`.
- Blocker: `ARTIFACT_PREFLIGHT_NO_RAW_SECRET_LOCK_MISSING`.
