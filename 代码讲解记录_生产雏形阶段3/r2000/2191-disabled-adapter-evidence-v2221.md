...
Windows root 统一替换为 `<repo>`，并将反斜杠改为正斜杠。批次末 Linux CI 又证明路径不是唯一宿主变量：
raw evidence metadata 会把 checkout 的 CRLF/LF 差异写入 `sizeBytes` 与 `digest`。portable 副本因此先读取
证据文本、统一为 LF 并重算元数据，再执行路径规范化。

规范化只作用于测试副本，不修改生产 profile。v2219/v2220 的本机 raw 摘要仍保留在归档，同时新增 portable
摘要；这样本机可以证明真实输出不变，CI 可以跨操作系统证明除路径和文本换行表示外的业务内容不变。

## Ready Oracle / local 与 fallback

固定时间后，portable ready JSON 为 21,502 字节，SHA-256
`be416997ac7103136b33e42d9c63b41376a315a9ea2a28aad1a41ac5793f58cb`；Markdown 为 21,115 字节，摘要
`4c657eee22c6a727d864051873c2cc2ddd83fd9da5244527bb06bbd585c6e4aa`。local 与 forced fallback 两次加载
