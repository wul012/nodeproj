...
portable ready 是 25,240 与 24,867 字节；blocked 也各有独立长度和摘要。portable 副本只把 repository
绝对根替换为 `<repo>` 并统一斜杠，不改变生产 profile，因此能在 Linux CI 上复验同一业务字节。
...
远端唯一 CI 在统一推送后验证 Linux、coverage、build 与 CI 自有 smoke。截图不适用，因为没有 HTML/UI
变化；关闭后不继续增加验证链，下一批从账本中的最高真实热点重新取证。
