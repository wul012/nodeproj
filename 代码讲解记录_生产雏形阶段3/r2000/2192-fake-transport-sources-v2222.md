...
远端首轮 CI 完成 Linux 静态门并定位 portable oracle 噪声；修复 run `29803696909` 随后覆盖 579 个测试文件、
1,765 项测试并全部通过，测试阶段耗时 830.61 秒。statements/branches/functions/lines coverage 分别为
95.67/87.95/98.52/95.64%，build、health、零请求 metrics、release readiness 的 archive-ready/禁止执行断言、
服务停止与 post-cleanup 全绿。截图不适用，因为没有 HTML/UI 变化；关闭后不继续增加验证链，下一批从账本中的
最高真实热点重新取证。
