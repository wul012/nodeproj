...
目标是用中文解释 Node v2105 的后期可读性保养代码路径：把 Types、Rules、Scanner、Gate、Tests 的命名族关系写清楚，并把 code walkthrough、f-folder、readability 三组门禁放到同一张表里。非目标是生产执行、真实批准、外部 artifact 导入、Java 或 mini-kv live service 启动、managed audit 写入、凭据读取。这个版本的价值在于把控制面阅读路径稳定下来，让后续维护者知道先看哪里、route 对应哪个 service、测试锁住什么边界。
