...
目标是用中文解释 Node v2104 的后期可读性保养代码路径：把 audit、access policy、route catalog、quality gate、shard preflight 五类控制面能力放进同一张地图。非目标是生产执行、真实批准、外部 artifact 导入、Java 或 mini-kv live service 启动、managed audit 写入、凭据读取。这个版本的价值在于把控制面阅读路径稳定下来，让后续维护者知道先看哪里、route 对应哪个 service、测试锁住什么边界。
