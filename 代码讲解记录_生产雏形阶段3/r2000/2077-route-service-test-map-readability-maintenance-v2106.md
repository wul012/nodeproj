...
目标是用中文解释 Node v2106 的后期可读性保养代码路径：把 managed-audit route quality 组里的 route、service、test 和只读边界放到一张对照表里。非目标是生产执行、真实批准、外部 artifact 导入、Java 或 mini-kv live service 启动、managed audit 写入、凭据读取。这个版本的价值在于把控制面阅读路径稳定下来，让后续维护者知道先看哪里、route 对应哪个 service、测试锁住什么边界。
...
本次修正不是继续把 代码讲解 拉长，而是把 route-service-test 对照表 的解释拆成可扫描的小节。route、service 和 test 的对应关系分散在多个目录，读者很难从一个 HTTP 入口追到负责生成 profile 的代码。 这类问题如果只靠一个大段落描述，审查者会很难判断它对应的是 docs/architecture/route-service-test-map.md、src/services/readabilityMaintenanceProfile.ts 还是 test/auditJsonMarkdownRouteCatalogSummary.test.ts。所以这篇文档保留必要深度，但把长段落拆成问题、入口、边界、验证和停止条件几个方向。
