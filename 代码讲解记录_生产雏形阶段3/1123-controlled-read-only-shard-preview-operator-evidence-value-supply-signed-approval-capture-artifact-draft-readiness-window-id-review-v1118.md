# Code walkthrough 1123: Node v1118 capture window id review lane

This walkthrough explains how Node v1118 participates in the signed approval artifact draft readiness package. The implementation is intentionally split so the long governance chain stays maintainable instead of turning the profile service into one giant file.
...
- v1118 lane: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_READINESS_WINDOW_ID_REVIEW`.
- Source field checked by builder: `captureWindowId`.
- Blocker emitted by control catalog: `REJECT_DRAFT_READINESS_WINDOW_ID_UNREVIEWED`.
