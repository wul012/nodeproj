# Node v906 code walkthrough: operator evidence value draft Node target

v906 contributes `VALUE_DRAFT_NODE_TARGET` to the operator evidence value draft package. It maps back to `IMPORT_PREFLIGHT_NODE_TARGET` and adds Node target status draft labels without accepting actual values.
