# Node v857 code walkthrough: manual evidence entry worksheet Java target

v857 contributes `ENTRY_WORKSHEET_JAVA_TARGET` to the blank manual evidence entry worksheet. It maps back to `INTAKE_REVIEW_JAVA_TARGET` from the v836 review package and adds the Java target status fields while stating Node must not start Java.
