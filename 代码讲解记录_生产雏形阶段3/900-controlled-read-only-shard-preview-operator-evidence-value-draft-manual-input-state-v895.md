# Node v895 code walkthrough: operator evidence value draft manual input state

v895 contributes `VALUE_DRAFT_MANUAL_INPUT_STATE` to the operator evidence value draft package. It maps back to `IMPORT_PREFLIGHT_MANUAL_INPUT_STATE` and adds manual input state draft labels without accepting actual values.
