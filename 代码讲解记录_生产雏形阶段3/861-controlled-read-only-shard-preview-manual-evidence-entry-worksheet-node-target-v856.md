# Node v856 code walkthrough: manual evidence entry worksheet Node target

v856 contributes `ENTRY_WORKSHEET_NODE_TARGET` to the blank manual evidence entry worksheet. It maps back to `INTAKE_REVIEW_NODE_TARGET` from the v836 review package and adds the Node target status fields for later operator-provided health and Markdown evidence.
