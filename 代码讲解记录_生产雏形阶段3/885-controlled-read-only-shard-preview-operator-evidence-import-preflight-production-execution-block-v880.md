# Node v880 code walkthrough: operator evidence import preflight production execution block

v880 contributes `IMPORT_PREFLIGHT_PRODUCTION_EXECUTION_BLOCK` to the operator evidence import preflight. It maps back to `ENTRY_WORKSHEET_PRODUCTION_EXECUTION_BLOCK` from the v861 worksheet and keeps production execution blocked even when preflight structure is ready.
