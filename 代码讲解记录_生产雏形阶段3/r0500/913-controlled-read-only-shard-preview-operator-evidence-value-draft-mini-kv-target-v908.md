# Node v908 code walkthrough: operator evidence value draft mini-kv target

v908 contributes `VALUE_DRAFT_MINI_KV_TARGET` to the operator evidence value draft package. It maps back to `IMPORT_PREFLIGHT_MINI_KV_TARGET` and adds mini-kv target status draft labels without accepting actual values.
