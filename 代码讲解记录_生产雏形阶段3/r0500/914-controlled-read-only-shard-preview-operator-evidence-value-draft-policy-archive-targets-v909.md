# Node v909 code walkthrough: operator evidence value draft policy/archive targets

v909 contributes `VALUE_DRAFT_POLICY_ARCHIVE_TARGETS` to the operator evidence value draft package. It maps back to `IMPORT_PREFLIGHT_POLICY_ARCHIVE_TARGETS` and adds policy/archive digest reference draft fields without accepting actual values.
