# Node v845 code walkthrough: manual evidence entry worksheet manual input state

v845 contributes `ENTRY_WORKSHEET_MANUAL_INPUT_STATE` to the blank manual evidence entry worksheet. It maps back to `INTAKE_REVIEW_MANUAL_INPUT_STATE` from the v836 review package and locks the worksheet to not-entered manual state instead of silently accepting values.
