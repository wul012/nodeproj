# Node v848 code walkthrough: manual evidence entry worksheet target coverage

v848 contributes `ENTRY_WORKSHEET_TARGET_COVERAGE` to the blank manual evidence entry worksheet. It maps back to `INTAKE_REVIEW_TARGET_COVERAGE` from the v836 review package and records target scope coverage before Node, Java, mini-kv, and policy/archive slots can be filled later.
