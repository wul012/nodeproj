# Node v852 code walkthrough: manual evidence entry worksheet read-only boundary

v852 contributes `ENTRY_WORKSHEET_READ_ONLY_BOUNDARY` to the blank manual evidence entry worksheet. It maps back to `INTAKE_REVIEW_READ_ONLY_BOUNDARY` from the v836 review package and records read-only control count and blocks non-read-only slots.
