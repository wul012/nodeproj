# Node v900 code walkthrough: operator evidence value draft failure class coverage

v900 contributes `VALUE_DRAFT_FAILURE_CLASS_COVERAGE` to the operator evidence value draft package. It maps back to `IMPORT_PREFLIGHT_FAILURE_CLASS_COVERAGE` and adds failure class coverage draft labels without accepting actual values.
