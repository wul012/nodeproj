# Node v838 code walkthrough: manual evidence entry worksheet control count

v838 contributes `ENTRY_WORKSHEET_CONTROL_COUNT` to the blank manual evidence entry worksheet. It maps back to `INTAKE_REVIEW_CONTROL_COUNT_COMPLETE` from the v836 review package and records observed and expected review control counts so manual entry cannot proceed on a partial package.
