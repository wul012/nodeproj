# Node v899 code walkthrough: operator evidence value draft cleanup coverage

v899 contributes `VALUE_DRAFT_CLEANUP_COVERAGE` to the operator evidence value draft package. It maps back to `IMPORT_PREFLIGHT_CLEANUP_COVERAGE` and adds cleanup coverage draft labels without accepting actual values.
