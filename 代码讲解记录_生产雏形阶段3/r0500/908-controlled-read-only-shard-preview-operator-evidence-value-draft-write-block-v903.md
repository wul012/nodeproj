# Node v903 code walkthrough: operator evidence value draft write block

v903 contributes `VALUE_DRAFT_WRITE_BLOCK` to the operator evidence value draft package. It maps back to `IMPORT_PREFLIGHT_WRITE_BLOCK` and adds write block draft confirmations without accepting actual values.
