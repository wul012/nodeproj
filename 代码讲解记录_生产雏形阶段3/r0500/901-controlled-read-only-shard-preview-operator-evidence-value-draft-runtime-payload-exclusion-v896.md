# Node v896 code walkthrough: operator evidence value draft runtime payload exclusion

v896 contributes `VALUE_DRAFT_RUNTIME_PAYLOAD_EXCLUSION` to the operator evidence value draft package. It maps back to `IMPORT_PREFLIGHT_RUNTIME_PAYLOAD_EXCLUSION` and adds runtime payload exclusion draft flags without accepting actual values.
