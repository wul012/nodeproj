# Node v882 code walkthrough: operator evidence import preflight Java target

v882 contributes `IMPORT_PREFLIGHT_JAVA_TARGET` to the operator evidence import preflight. It maps back to `ENTRY_WORKSHEET_JAVA_TARGET` from the v861 worksheet and defines Java status normalization while preventing Node from starting Java.
