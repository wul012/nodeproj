# Node v870 code walkthrough: operator evidence import preflight manual input state

v870 contributes `IMPORT_PREFLIGHT_MANUAL_INPUT_STATE` to the operator evidence import preflight. It maps back to `ENTRY_WORKSHEET_MANUAL_INPUT_STATE` from the v861 worksheet and keeps manual input state canonical while values remain not imported.
