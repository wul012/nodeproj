# Node v884 code walkthrough: operator evidence import preflight policy/archive targets

v884 contributes `IMPORT_PREFLIGHT_POLICY_ARCHIVE_TARGETS` to the operator evidence import preflight. It maps back to `ENTRY_WORKSHEET_POLICY_ARCHIVE_TARGETS` from the v861 worksheet and defines policy and archive status normalization without runtime payload.
