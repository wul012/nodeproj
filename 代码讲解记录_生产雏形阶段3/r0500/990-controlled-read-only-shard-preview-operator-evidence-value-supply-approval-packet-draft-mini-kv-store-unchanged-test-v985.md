# Code walkthrough 990: Node v985 approval packet draft mini-kv store unchanged test

## Version Position

This walkthrough covers Node v985 in the v962-v986 approval packet draft batch. The slice consumes mini-kv-v610-value-supply-test / mini-kv-value-supply-test-store-unchanged and maps it from source envelope slot VALUE_SUPPLY_ENVELOPE_MINI_KV_NO_WRITE_TEST.
...
The artifact builder loads historical fallback-aware files, matches snippets, asks the slot builder to map Node v961 envelope slots to v962-v986 approval draft slots, applies policy catalog fields, calculates fail-closed gates, and emits a stable digest. v985 becomes ready only when its evidence file and snippet match and the source envelope slot is already ready.
