# Node v846 code walkthrough: manual evidence entry worksheet runtime payload exclusion

v846 contributes `ENTRY_WORKSHEET_RUNTIME_PAYLOAD_EXCLUSION` to the blank manual evidence entry worksheet. It maps back to `INTAKE_REVIEW_RUNTIME_PAYLOAD_EXCLUSION` from the v836 review package and records runtime payload exclusion and keeps imported runtime payload count at zero.
