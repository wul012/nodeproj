# Node v898 code walkthrough: operator evidence value draft target coverage

v898 contributes `VALUE_DRAFT_TARGET_COVERAGE` to the operator evidence value draft package. It maps back to `IMPORT_PREFLIGHT_TARGET_COVERAGE` and adds Node, Java, and mini-kv target coverage draft counts without accepting actual values.
