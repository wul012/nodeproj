# Node v891 code walkthrough: operator evidence value draft pending capture state

v891 contributes `VALUE_DRAFT_PENDING_CAPTURE_STATE` to the operator evidence value draft package. It maps back to `IMPORT_PREFLIGHT_PENDING_CAPTURE_STATE` and adds pending capture state draft labels without accepting actual values.
