# Node v905 code walkthrough: operator evidence value draft production execution block

v905 contributes `VALUE_DRAFT_PRODUCTION_EXECUTION_BLOCK` to the operator evidence value draft package. It maps back to `IMPORT_PREFLIGHT_PRODUCTION_EXECUTION_BLOCK` and adds production execution block draft confirmations without accepting actual values.
