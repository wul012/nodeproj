# Node v890 code walkthrough: operator evidence value draft source record mapping

v890 contributes `VALUE_DRAFT_SOURCE_RECORD_MAPPING` to the operator evidence value draft package. It maps back to `IMPORT_PREFLIGHT_SOURCE_RECORD_MAPPING` and adds source mapping draft counts without accepting actual values.
