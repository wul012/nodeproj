# Node v902 code walkthrough: operator evidence value draft read-only boundary

v902 contributes `VALUE_DRAFT_READ_ONLY_BOUNDARY` to the operator evidence value draft package. It maps back to `IMPORT_PREFLIGHT_READ_ONLY_BOUNDARY` and adds read-only boundary draft confirmations without accepting actual values.
