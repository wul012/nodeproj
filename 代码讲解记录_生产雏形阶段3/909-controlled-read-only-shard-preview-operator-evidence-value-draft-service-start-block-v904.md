# Node v904 code walkthrough: operator evidence value draft service start block

v904 contributes `VALUE_DRAFT_SERVICE_START_BLOCK` to the operator evidence value draft package. It maps back to `IMPORT_PREFLIGHT_SERVICE_START_BLOCK` and adds service start block draft confirmations without accepting actual values.
