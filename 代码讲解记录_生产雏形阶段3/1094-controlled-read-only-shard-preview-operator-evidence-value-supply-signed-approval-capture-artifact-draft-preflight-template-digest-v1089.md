# Code walkthrough: Node v1089 signed approval capture artifact draft preflight template digest binding
...
Node v1089 contributes template digest binding. The code keeps readyForSignedApprovalArtifactDraft=false, readyForSignedApprovalCapture=false, readyForOperatorValueSupply=false, readyForEvidenceImport=false, readyForRuntimePayload=false, and executionAllowed=false.

## Version-Specific Mapping

- Field: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_PREFLIGHT_TEMPLATE_DIGEST`.
- Guard: `ARTIFACT_DRAFT_PREFLIGHT_TEMPLATE_DIGEST_GUARD`.
- Source artifact fragment: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_PREFLIGHT_TEMPLATE_DIGEST`.
- Required artifact fragment: `sourceSignedApprovalTemplateDigest`.
- Blocker: `ARTIFACT_DRAFT_PREFLIGHT_TEMPLATE_DIGEST_MISSING`.
