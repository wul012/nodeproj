# Code walkthrough: Node v1106 signed approval capture artifact draft preflight no raw secret draft lock
...
Node v1106 contributes no raw secret draft lock. The code keeps readyForSignedApprovalArtifactDraft=false, readyForSignedApprovalCapture=false, readyForOperatorValueSupply=false, readyForEvidenceImport=false, readyForRuntimePayload=false, and executionAllowed=false.

## Version-Specific Mapping

- Field: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_PREFLIGHT_NO_RAW_SECRET_LOCK`.
- Guard: `ARTIFACT_DRAFT_PREFLIGHT_NO_RAW_SECRET_GUARD`.
- Source artifact fragment: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_PREFLIGHT_NO_RAW_SECRET_LOCK`.
- Required artifact fragment: `noRawSecretLock`.
- Blocker: `ARTIFACT_DRAFT_PREFLIGHT_NO_RAW_SECRET_LOCK_MISSING`.
