# Code walkthrough 1134: Node v1129 redaction policy review lane

This walkthrough explains how Node v1129 participates in the signed approval artifact draft readiness package. The implementation is intentionally split so the long governance chain stays maintainable instead of turning the profile service into one giant file.
...
- v1129 lane: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_READINESS_REDACTION_POLICY_REVIEW`.
- Source field checked by builder: `redactionPolicy`.
- Blocker emitted by control catalog: `REJECT_DRAFT_READINESS_REDACTION_POLICY_UNREVIEWED`.
