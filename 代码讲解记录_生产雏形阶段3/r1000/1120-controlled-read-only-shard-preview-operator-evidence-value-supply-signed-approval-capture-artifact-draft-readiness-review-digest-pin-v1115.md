# Code walkthrough 1120: Node v1115 approval review digest pin lane

This walkthrough explains how Node v1115 participates in the signed approval artifact draft readiness package. The implementation is intentionally split so the long governance chain stays maintainable instead of turning the profile service into one giant file.
...
- v1115 lane: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_READINESS_REVIEW_DIGEST_PIN`.
- Source field checked by builder: `sourceApprovalPacketReviewDigest`.
- Blocker emitted by control catalog: `REJECT_DRAFT_READINESS_REVIEW_DIGEST_UNPINNED`.
