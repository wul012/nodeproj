# Code walkthrough: Node v1063 signed approval capture artifact preflight source capture preflight digest binding
...
Node v1063 contributes source capture preflight digest binding. The code keeps readyForSignedApprovalCapture=false, readyForOperatorValueSupply=false, readyForEvidenceImport=false, readyForRuntimePayload=false, and executionAllowed=false.

## Version-Specific Mapping

- Fragment: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_PREFLIGHT_CAPTURE_PREFLIGHT_DIGEST`.
- Seal: `ARTIFACT_PREFLIGHT_CAPTURE_PREFLIGHT_DIGEST_SEAL`.
- Source capture input: `SIGNED_APPROVAL_CAPTURE_PREFLIGHT_TEMPLATE_DIGEST`.
- Required capture field: `sourceSignedApprovalTemplateDigest`.
- Blocker: `ARTIFACT_PREFLIGHT_CAPTURE_PREFLIGHT_DIGEST_MISSING`.
