# Code walkthrough 1130: Node v1125 source evidence file review lane

This walkthrough explains how Node v1125 participates in the signed approval artifact draft readiness package. The implementation is intentionally split so the long governance chain stays maintainable instead of turning the profile service into one giant file.
...
- v1125 lane: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_READINESS_SOURCE_FILE_REVIEW`.
- Source field checked by builder: `sourceEvidenceFileId`.
- Blocker emitted by control catalog: `REJECT_DRAFT_READINESS_SOURCE_FILE_UNREVIEWED`.
