# Code walkthrough 1119: Node v1114 template digest pin lane

This walkthrough explains how Node v1114 participates in the signed approval artifact draft readiness package. The implementation is intentionally split so the long governance chain stays maintainable instead of turning the profile service into one giant file.
...
- v1114 lane: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_READINESS_TEMPLATE_DIGEST_PIN`.
- Source field checked by builder: `sourceSignedApprovalTemplateDigest`.
- Blocker emitted by control catalog: `REJECT_DRAFT_READINESS_TEMPLATE_DIGEST_UNPINNED`.
