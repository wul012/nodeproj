# Code walkthrough 1135: Node v1130 provenance policy review lane

This walkthrough explains how Node v1130 participates in the signed approval artifact draft readiness package. The implementation is intentionally split so the long governance chain stays maintainable instead of turning the profile service into one giant file.
...
- v1130 lane: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_READINESS_PROVENANCE_POLICY_REVIEW`.
- Source field checked by builder: `provenancePolicy`.
- Blocker emitted by control catalog: `REJECT_DRAFT_READINESS_PROVENANCE_POLICY_UNREVIEWED`.
