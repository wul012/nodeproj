# Code walkthrough 1140: Node v1135 sibling non-mutation evidence lane

This walkthrough explains how Node v1135 participates in the signed approval artifact draft readiness package. The implementation is intentionally split so the long governance chain stays maintainable instead of turning the profile service into one giant file.
...
- v1135 lane: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_READINESS_SIBLING_NON_MUTATION_EVIDENCE`.
- Source field checked by builder: `siblingNonMutationLock`.
- Blocker emitted by control catalog: `REJECT_DRAFT_READINESS_SIBLING_MUTATION_ENABLED`.
