# Code walkthrough 1128: Node v1123 approval statement digest pin lane

This walkthrough explains how Node v1123 participates in the signed approval artifact draft readiness package. The implementation is intentionally split so the long governance chain stays maintainable instead of turning the profile service into one giant file.
...
- v1123 lane: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_READINESS_APPROVAL_STATEMENT_DIGEST_PIN`.
- Source field checked by builder: `approvalStatementDigest`.
- Blocker emitted by control catalog: `REJECT_DRAFT_READINESS_APPROVAL_STATEMENT_DIGEST_UNPINNED`.
