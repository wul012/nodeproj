# v844 Controlled read-only shard preview manual evidence entry worksheet redaction rules

v844 adds `ENTRY_WORKSHEET_REDACTION_RULES` to the manual evidence entry worksheet chain. It consumes `INTAKE_REVIEW_REDACTION_RULES_PRESERVED` from the v836 evidence intake review package and records redaction rule coverage and keeps secret values out of the worksheet.
...
No screenshot is needed because v844 adds service artifacts and Markdown route content without a new browser page.
