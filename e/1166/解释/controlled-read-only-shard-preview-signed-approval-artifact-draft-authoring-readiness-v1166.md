# Node v1166 signed approval artifact draft authoring readiness explanation

Node v1166 adds the operator identity authoring requirement for the controlled read-only shard preview signed approval chain.

## What changed

- Authoring requirement: SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_AUTHORING_READINESS_OPERATOR_IDENTITY
- Authoring blocker: SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_AUTHORING_READINESS_OPERATOR_IDENTITY_BLOCKER
- Category: identity
- Source binding: operator identity package slot and guard from Node v1161 review package preflight.
- Purpose: exposes reviewed operator identity as an authoring requirement while credentials and signatures remain absent.
