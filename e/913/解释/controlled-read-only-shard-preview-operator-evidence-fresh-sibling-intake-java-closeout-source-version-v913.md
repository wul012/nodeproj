# Node v913 controlled read-only shard preview operator evidence fresh sibling intake: Java closeout source version

## What This Version Owns

v913 contributes Java closeout source version to the Node v912-v936 fresh sibling intake package. The slice is owned by Java evidence consumption, but the implementation lives only in Node and reads committed sibling files through historical fallback-aware evidence helpers.

Detail: records that the Java closeout response remains pinned to Java v607 while the repository closeout tag is v608.
