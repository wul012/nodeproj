# Node v1209 signed approval artifact draft instruction preflight explanation

Node v1209 adds the write route embargo instruction slot for the controlled read-only shard preview signed approval chain.

## What changed

- Instruction slot: SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_INSTRUCTION_PREFLIGHT_WRITE_ROUTE_EMBARGO_SLOT
- Instruction guard: SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_INSTRUCTION_PREFLIGHT_WRITE_ROUTE_EMBARGO_SLOT_GUARD
- Category: execution lock
- Source binding: write route embargo authoring requirement and blocker from Node v1186 authoring readiness.
- Purpose: prepares an execution-lock instruction slot keeping write routes disabled.
