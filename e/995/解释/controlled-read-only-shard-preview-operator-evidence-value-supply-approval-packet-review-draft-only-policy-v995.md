# Node v995 controlled read-only shard preview operator evidence value supply approval packet review: draft-only no-grant policy review control

## What This Version Owns

v995 owns the draft-only no-grant policy review control slice of the v987-v1011 approval packet review package. It adds review control VALUE_SUPPLY_APPROVAL_REVIEW_DRAFT_ONLY_POLICY in the policy group and maps it to source approval packet draft slot VALUE_SUPPLY_APPROVAL_PACKET_JAVA_IMPORT_BLOCK.

## Source Draft Binding

- Source Node version: Node v986
- Source approval packet draft slot: VALUE_SUPPLY_APPROVAL_PACKET_JAVA_IMPORT_BLOCK
- Review control code: VALUE_SUPPLY_APPROVAL_REVIEW_DRAFT_ONLY_POLICY
