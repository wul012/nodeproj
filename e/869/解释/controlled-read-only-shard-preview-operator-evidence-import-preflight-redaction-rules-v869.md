# v869 Controlled read-only shard preview operator evidence import preflight redaction rules

v869 adds `IMPORT_PREFLIGHT_REDACTION_RULES` to the operator evidence import preflight chain. It consumes `ENTRY_WORKSHEET_REDACTION_RULES` from the v861 manual evidence entry worksheet and preserves redaction coverage and blocks secret-bearing values.
...
No screenshot is needed because v869 adds service artifacts and Markdown route content without a new browser page.
