# Node v929 controlled read-only shard preview operator evidence fresh sibling intake: mini-kv zero imported values

## What This Version Owns

v929 contributes mini-kv zero imported values to the Node v912-v936 fresh sibling intake package. The slice is owned by mini-kv evidence consumption, but the implementation lives only in Node and reads committed sibling files through historical fallback-aware evidence helpers.

Detail: matches importedEvidenceValueCount:0 and keeps actual value import out of scope.
