# Node v1115 controlled read-only shard preview signed approval artifact draft readiness: approval review digest pin lane

Node v1115 owns the approval packet review digest pin slice of the v1112-v1136 signed approval artifact draft readiness package. This is not a small archive-only marker: it adds a route-visible readiness lane, a matching readiness control, source draft preflight bindings, blocker semantics, gate coverage, renderer output, profile output, and focused test expectations.

## Functional Change

- Lane code: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_READINESS_REVIEW_DIGEST_PIN`.
- Lane kind: digest.
- Source v1111 draft preflight field: `sourceApprovalPacketReviewDigest`.
- Control blocker: `REJECT_DRAFT_READINESS_REVIEW_DIGEST_UNPINNED`.
