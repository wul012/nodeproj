# Node v1113 controlled read-only shard preview signed approval artifact draft readiness: artifact preflight digest pin lane

Node v1113 owns the v1086 artifact preflight digest pin slice of the v1112-v1136 signed approval artifact draft readiness package. This is not a small archive-only marker: it adds a route-visible readiness lane, a matching readiness control, source draft preflight bindings, blocker semantics, gate coverage, renderer output, profile output, and focused test expectations.

## Functional Change

- Lane code: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_READINESS_ARTIFACT_PREFLIGHT_DIGEST_PIN`.
- Lane kind: digest.
- Source v1111 draft preflight field: `sourceSignedApprovalCaptureArtifactPreflightDigest`.
- Control blocker: `REJECT_DRAFT_READINESS_ARTIFACT_PREFLIGHT_DIGEST_UNPINNED`.
