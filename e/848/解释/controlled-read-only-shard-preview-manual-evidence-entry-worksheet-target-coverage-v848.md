# v848 Controlled read-only shard preview manual evidence entry worksheet target coverage

v848 adds `ENTRY_WORKSHEET_TARGET_COVERAGE` to the manual evidence entry worksheet chain. It consumes `INTAKE_REVIEW_TARGET_COVERAGE` from the v836 evidence intake review package and records target scope coverage before Node, Java, mini-kv, and policy/archive slots can be filled later.
...
No screenshot is needed because v848 adds service artifacts and Markdown route content without a new browser page.
