# Node v1205 signed approval artifact draft instruction preflight explanation

Node v1205 adds the provenance policy instruction slot for the controlled read-only shard preview signed approval chain.

## What changed

- Instruction slot: SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_INSTRUCTION_PREFLIGHT_PROVENANCE_POLICY_SLOT
- Instruction guard: SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_INSTRUCTION_PREFLIGHT_PROVENANCE_POLICY_SLOT_GUARD
- Category: policy
- Source binding: provenance policy authoring requirement and blocker from Node v1186 authoring readiness.
- Purpose: prepares a policy instruction slot for provenance policy.
