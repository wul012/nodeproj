# v879 Controlled read-only shard preview operator evidence import preflight service start block

v879 adds `IMPORT_PREFLIGHT_SERVICE_START_BLOCK` to the operator evidence import preflight chain. It consumes `ENTRY_WORKSHEET_SERVICE_START_BLOCK` from the v861 manual evidence entry worksheet and keeps service startup blocked for Node, Java, and mini-kv.
...
No screenshot is needed because v879 adds service artifacts and Markdown route content without a new browser page.
