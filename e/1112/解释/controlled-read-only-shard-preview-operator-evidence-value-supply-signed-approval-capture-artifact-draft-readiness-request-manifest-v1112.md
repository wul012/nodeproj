# Node v1112 controlled read-only shard preview signed approval artifact draft readiness: request manifest readiness lane

Node v1112 owns the stable request manifest binding slice of the v1112-v1136 signed approval artifact draft readiness package. This is not a small archive-only marker: it adds a route-visible readiness lane, a matching readiness control, source draft preflight bindings, blocker semantics, gate coverage, renderer output, profile output, and focused test expectations.

## Functional Change

- Lane code: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_READINESS_REQUEST_MANIFEST`.
- Lane kind: identity.
- Source v1111 draft preflight field: `draftArtifactRequestId`.
- Control blocker: `REJECT_DRAFT_READINESS_REQUEST_MANIFEST_MISSING`.
