# v854 Controlled read-only shard preview manual evidence entry worksheet service start block

v854 adds `ENTRY_WORKSHEET_SERVICE_START_BLOCK` to the manual evidence entry worksheet chain. It consumes `INTAKE_REVIEW_SERVICE_START_BLOCK` from the v836 evidence intake review package and records service-start absence and prevents Node from starting Java or mini-kv.
...
No screenshot is needed because v854 adds service artifacts and Markdown route content without a new browser page.
