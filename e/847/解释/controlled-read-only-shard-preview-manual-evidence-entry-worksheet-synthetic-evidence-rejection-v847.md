# v847 Controlled read-only shard preview manual evidence entry worksheet synthetic evidence rejection

v847 adds `ENTRY_WORKSHEET_SYNTHETIC_EVIDENCE_REJECTION` to the manual evidence entry worksheet chain. It consumes `INTAKE_REVIEW_SYNTHETIC_EVIDENCE_REJECTION` from the v836 evidence intake review package and records that synthetic evidence is not accepted by this blank worksheet package.
...
No screenshot is needed because v847 adds service artifacts and Markdown route content without a new browser page.
