# Node v917 controlled read-only shard preview operator evidence fresh sibling intake: Java evidence import block

## What This Version Owns

v917 contributes Java evidence import block to the Node v912-v936 fresh sibling intake package. The slice is owned by Java evidence consumption, but the implementation lives only in Node and reads committed sibling files through historical fallback-aware evidence helpers.

Detail: freezes readyForEvidenceImport so Node can assert the value import path is still false.
