# Node v1162 signed approval artifact draft authoring readiness explanation

Node v1162 adds the request manifest authoring requirement for the controlled read-only shard preview signed approval chain.

## What changed

- Authoring requirement: SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_AUTHORING_READINESS_REQUEST_MANIFEST
- Authoring blocker: SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_AUTHORING_READINESS_REQUEST_MANIFEST_BLOCKER
- Category: identity
- Source binding: request manifest package slot and guard from Node v1161 review package preflight.
- Purpose: binds the reviewed request manifest slot into a human draft authoring requirement without materializing instructions.
