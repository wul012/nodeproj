# v856 Controlled read-only shard preview manual evidence entry worksheet Node target

v856 adds `ENTRY_WORKSHEET_NODE_TARGET` to the manual evidence entry worksheet chain. It consumes `INTAKE_REVIEW_NODE_TARGET` from the v836 evidence intake review package and adds the Node target status fields for later operator-provided health and Markdown evidence.
...
No screenshot is needed because v856 adds service artifacts and Markdown route content without a new browser page.
