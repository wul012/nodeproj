# v893 Controlled read-only shard preview operator evidence value draft acceptance criteria

v893 adds `VALUE_DRAFT_ACCEPTANCE_CRITERIA` to the operator evidence value draft chain. It consumes `IMPORT_PREFLIGHT_ACCEPTANCE_CRITERIA` from the v862-v886 import preflight and contributes acceptance criterion draft statuses.
...
- draft code: `VALUE_DRAFT_ACCEPTANCE_CRITERIA`;
- source preflight slot: `IMPORT_PREFLIGHT_ACCEPTANCE_CRITERIA`;
- focus: acceptance criterion draft statuses;
- boundary: keeps acceptance rules inherited from the preflight.
...
No screenshot is needed because v893 adds service artifacts and Markdown route content without a new browser page.
