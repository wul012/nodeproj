# v870 Controlled read-only shard preview operator evidence import preflight manual input state

v870 adds `IMPORT_PREFLIGHT_MANUAL_INPUT_STATE` to the operator evidence import preflight chain. It consumes `ENTRY_WORKSHEET_MANUAL_INPUT_STATE` from the v861 manual evidence entry worksheet and keeps manual input state canonical while values remain not imported.
...
No screenshot is needed because v870 adds service artifacts and Markdown route content without a new browser page.
