# v884 Controlled read-only shard preview operator evidence import preflight policy/archive targets

v884 adds `IMPORT_PREFLIGHT_POLICY_ARCHIVE_TARGETS` to the operator evidence import preflight chain. It consumes `ENTRY_WORKSHEET_POLICY_ARCHIVE_TARGETS` from the v861 manual evidence entry worksheet and defines policy and archive status normalization without runtime payload.
...
No screenshot is needed because v884 adds service artifacts and Markdown route content without a new browser page.
