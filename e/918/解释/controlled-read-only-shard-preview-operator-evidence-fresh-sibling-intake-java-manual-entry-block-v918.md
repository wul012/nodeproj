# Node v918 controlled read-only shard preview operator evidence fresh sibling intake: Java manual entry block

## What This Version Owns

v918 contributes Java manual entry block to the Node v912-v936 fresh sibling intake package. The slice is owned by Java evidence consumption, but the implementation lives only in Node and reads committed sibling files through historical fallback-aware evidence helpers.

Detail: freezes readyForManualEvidenceEntry so manual value entry remains out of scope.
