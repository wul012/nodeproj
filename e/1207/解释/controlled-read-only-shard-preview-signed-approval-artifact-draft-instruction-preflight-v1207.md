# Node v1207 signed approval artifact draft instruction preflight explanation

Node v1207 adds the approval grant embargo instruction slot for the controlled read-only shard preview signed approval chain.

## What changed

- Instruction slot: SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_INSTRUCTION_PREFLIGHT_APPROVAL_GRANT_EMBARGO_SLOT
- Instruction guard: SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_INSTRUCTION_PREFLIGHT_APPROVAL_GRANT_EMBARGO_SLOT_GUARD
- Category: execution lock
- Source binding: approval grant embargo authoring requirement and blocker from Node v1186 authoring readiness.
- Purpose: prepares an execution-lock instruction slot proving no approval grant exists.
