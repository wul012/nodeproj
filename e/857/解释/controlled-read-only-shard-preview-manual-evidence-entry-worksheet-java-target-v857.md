# v857 Controlled read-only shard preview manual evidence entry worksheet Java target

v857 adds `ENTRY_WORKSHEET_JAVA_TARGET` to the manual evidence entry worksheet chain. It consumes `INTAKE_REVIEW_JAVA_TARGET` from the v836 evidence intake review package and adds the Java target status fields while stating Node must not start Java.
...
No screenshot is needed because v857 adds service artifacts and Markdown route content without a new browser page.
