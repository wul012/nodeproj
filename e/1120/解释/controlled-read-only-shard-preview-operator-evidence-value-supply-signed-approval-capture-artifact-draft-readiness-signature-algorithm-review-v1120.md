# Node v1120 controlled read-only shard preview signed approval artifact draft readiness: signature algorithm review lane

Node v1120 owns the detached signature algorithm policy slice of the v1112-v1136 signed approval artifact draft readiness package. This is not a small archive-only marker: it adds a route-visible readiness lane, a matching readiness control, source draft preflight bindings, blocker semantics, gate coverage, renderer output, profile output, and focused test expectations.

## Functional Change

- Lane code: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_READINESS_SIGNATURE_ALGORITHM_REVIEW`.
- Lane kind: signature.
- Source v1111 draft preflight field: `signatureAlgorithmPolicy`.
- Control blocker: `REJECT_DRAFT_READINESS_SIGNATURE_ALGORITHM_UNREVIEWED`.
