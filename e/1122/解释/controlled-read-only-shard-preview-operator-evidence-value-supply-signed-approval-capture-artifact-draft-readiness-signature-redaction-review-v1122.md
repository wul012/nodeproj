# Node v1122 controlled read-only shard preview signed approval artifact draft readiness: signature redaction review lane

Node v1122 owns the signature material redaction policy slice of the v1112-v1136 signed approval artifact draft readiness package. This is not a small archive-only marker: it adds a route-visible readiness lane, a matching readiness control, source draft preflight bindings, blocker semantics, gate coverage, renderer output, profile output, and focused test expectations.

## Functional Change

- Lane code: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_READINESS_SIGNATURE_REDACTION_REVIEW`.
- Lane kind: signature.
- Source v1111 draft preflight field: `signatureMaterialRedactionPolicy`.
- Control blocker: `REJECT_DRAFT_READINESS_SIGNATURE_REDACTION_UNREVIEWED`.
