# v881 Controlled read-only shard preview operator evidence import preflight Node target

v881 adds `IMPORT_PREFLIGHT_NODE_TARGET` to the operator evidence import preflight chain. It consumes `ENTRY_WORKSHEET_NODE_TARGET` from the v861 manual evidence entry worksheet and defines Node target status normalization without raw payload import.
...
No screenshot is needed because v881 adds service artifacts and Markdown route content without a new browser page.
