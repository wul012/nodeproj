# Node v1164 signed approval artifact draft authoring readiness explanation

Node v1164 adds the template digest authoring requirement for the controlled read-only shard preview signed approval chain.

## What changed

- Authoring requirement: SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_AUTHORING_READINESS_TEMPLATE_DIGEST
- Authoring blocker: SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_AUTHORING_READINESS_TEMPLATE_DIGEST_BLOCKER
- Category: digest binding
- Source binding: template digest package slot and guard from Node v1161 review package preflight.
- Purpose: pins the signed approval template digest before any human signed approval wording exists.
