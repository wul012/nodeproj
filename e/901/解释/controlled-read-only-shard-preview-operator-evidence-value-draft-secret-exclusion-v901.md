# v901 Controlled read-only shard preview operator evidence value draft secret exclusion

v901 adds `VALUE_DRAFT_SECRET_EXCLUSION` to the operator evidence value draft chain. It consumes `IMPORT_PREFLIGHT_SECRET_EXCLUSION` from the v862-v886 import preflight and contributes secret exclusion draft confirmations.
...
- draft code: `VALUE_DRAFT_SECRET_EXCLUSION`;
- source preflight slot: `IMPORT_PREFLIGHT_SECRET_EXCLUSION`;
- focus: secret exclusion draft confirmations;
- boundary: rejects credential or token values.
...
No screenshot is needed because v901 adds service artifacts and Markdown route content without a new browser page.
