# v851 Controlled read-only shard preview manual evidence entry worksheet secret exclusion

v851 adds `ENTRY_WORKSHEET_SECRET_EXCLUSION` to the manual evidence entry worksheet chain. It consumes `INTAKE_REVIEW_SECRET_EXCLUSION` from the v836 evidence intake review package and records secret exclusion and keeps every slot free of secret values.
...
No screenshot is needed because v851 adds service artifacts and Markdown route content without a new browser page.
