# v909 Controlled read-only shard preview operator evidence value draft policy/archive targets

v909 adds `VALUE_DRAFT_POLICY_ARCHIVE_TARGETS` to the operator evidence value draft chain. It consumes `IMPORT_PREFLIGHT_POLICY_ARCHIVE_TARGETS` from the v862-v886 import preflight and contributes policy/archive digest reference draft fields.
...
- draft code: `VALUE_DRAFT_POLICY_ARCHIVE_TARGETS`;
- source preflight slot: `IMPORT_PREFLIGHT_POLICY_ARCHIVE_TARGETS`;
- focus: policy/archive digest reference draft fields;
- boundary: does not import archive payloads.
...
No screenshot is needed because v909 adds service artifacts and Markdown route content without a new browser page.
