# Node v1065 signed approval capture artifact preflight approval packet review digest binding

This archive records Node v1065 in the controlled read-only shard preview operator evidence value supply signed approval capture artifact preflight chain.

## Scope

- Adds or verifies the approval packet review digest binding part of the signed approval capture artifact preflight surface.
- Binds artifact fragment `SIGNED_APPROVAL_CAPTURE_ARTIFACT_PREFLIGHT_REVIEW_DIGEST` to source capture input `SIGNED_APPROVAL_CAPTURE_PREFLIGHT_REVIEW_DIGEST` and required capture input `sourceApprovalPacketReviewDigest`.
- Binds artifact seal `ARTIFACT_PREFLIGHT_REVIEW_DIGEST_SEAL` to blocker `ARTIFACT_PREFLIGHT_REVIEW_DIGEST_MISSING` so a future artifact draft cannot silently skip this requirement.
...
Java and mini-kv can continue in parallel. Node v1065 does not require fresh sibling evidence and is not a pre-approval blocker for their next work.
