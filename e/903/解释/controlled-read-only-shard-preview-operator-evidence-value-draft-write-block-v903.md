# v903 Controlled read-only shard preview operator evidence value draft write block

v903 adds `VALUE_DRAFT_WRITE_BLOCK` to the operator evidence value draft chain. It consumes `IMPORT_PREFLIGHT_WRITE_BLOCK` from the v862-v886 import preflight and contributes write block draft confirmations.
...
- draft code: `VALUE_DRAFT_WRITE_BLOCK`;
- source preflight slot: `IMPORT_PREFLIGHT_WRITE_BLOCK`;
- focus: write block draft confirmations;
- boundary: keeps routing promotion disabled.
...
No screenshot is needed because v903 adds service artifacts and Markdown route content without a new browser page.
