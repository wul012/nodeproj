# v846 Controlled read-only shard preview manual evidence entry worksheet runtime payload exclusion

v846 adds `ENTRY_WORKSHEET_RUNTIME_PAYLOAD_EXCLUSION` to the manual evidence entry worksheet chain. It consumes `INTAKE_REVIEW_RUNTIME_PAYLOAD_EXCLUSION` from the v836 evidence intake review package and records runtime payload exclusion and keeps imported runtime payload count at zero.
...
No screenshot is needed because v846 adds service artifacts and Markdown route content without a new browser page.
