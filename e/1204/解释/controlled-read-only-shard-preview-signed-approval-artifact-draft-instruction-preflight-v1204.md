# Node v1204 signed approval artifact draft instruction preflight explanation

Node v1204 adds the redaction policy instruction slot for the controlled read-only shard preview signed approval chain.

## What changed

- Instruction slot: SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_INSTRUCTION_PREFLIGHT_REDACTION_POLICY_SLOT
- Instruction guard: SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_INSTRUCTION_PREFLIGHT_REDACTION_POLICY_SLOT_GUARD
- Category: policy
- Source binding: redaction policy authoring requirement and blocker from Node v1186 authoring readiness.
- Purpose: prepares a policy instruction slot for redaction policy.
