# Node v1134 controlled read-only shard preview signed approval artifact draft readiness: write route embargo lane

Node v1134 owns the write route disabled guarantee slice of the v1112-v1136 signed approval artifact draft readiness package. This is not a small archive-only marker: it adds a route-visible readiness lane, a matching readiness control, source draft preflight bindings, blocker semantics, gate coverage, renderer output, profile output, and focused test expectations.

## Functional Change

- Lane code: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_READINESS_WRITE_ROUTE_EMBARGO`.
- Lane kind: execution lock.
- Source v1111 draft preflight field: `noWriteRouteLock`.
- Control blocker: `REJECT_DRAFT_READINESS_WRITE_ROUTE_ENABLED`.
