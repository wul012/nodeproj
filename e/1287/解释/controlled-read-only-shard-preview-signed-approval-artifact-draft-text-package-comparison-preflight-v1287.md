# Node v1287 signed approval artifact draft text package comparison preflight

## What changed

Node v1287 contributes to the v1287-v1311 comparison preflight by covering the identity slice: request manifest. It turns the corresponding v1286 submission slot into an offline comparison lane and pairs it with an acceptance control for uncompared or unacceptable package material.
...
The identity slice opens the offline comparison surface with the request manifest lane and an acceptance control that rejects uncompared package material before any signed draft text can be accepted.
