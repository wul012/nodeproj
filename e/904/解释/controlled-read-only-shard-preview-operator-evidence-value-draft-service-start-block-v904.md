# v904 Controlled read-only shard preview operator evidence value draft service start block

v904 adds `VALUE_DRAFT_SERVICE_START_BLOCK` to the operator evidence value draft chain. It consumes `IMPORT_PREFLIGHT_SERVICE_START_BLOCK` from the v862-v886 import preflight and contributes service start block draft confirmations.
...
- draft code: `VALUE_DRAFT_SERVICE_START_BLOCK`;
- source preflight slot: `IMPORT_PREFLIGHT_SERVICE_START_BLOCK`;
- focus: service start block draft confirmations;
- boundary: does not start Java, mini-kv, or Node preview servers.
...
No screenshot is needed because v904 adds service artifacts and Markdown route content without a new browser page.
