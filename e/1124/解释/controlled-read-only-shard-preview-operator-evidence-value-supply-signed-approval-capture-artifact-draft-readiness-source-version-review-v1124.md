# Node v1124 controlled read-only shard preview signed approval artifact draft readiness: source evidence version review lane

Node v1124 owns the source evidence version review slice of the v1112-v1136 signed approval artifact draft readiness package. This is not a small archive-only marker: it adds a route-visible readiness lane, a matching readiness control, source draft preflight bindings, blocker semantics, gate coverage, renderer output, profile output, and focused test expectations.

## Functional Change

- Lane code: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_READINESS_SOURCE_VERSION_REVIEW`.
- Lane kind: source evidence.
- Source v1111 draft preflight field: `sourceEvidenceVersion`.
- Control blocker: `REJECT_DRAFT_READINESS_SOURCE_VERSION_UNREVIEWED`.
