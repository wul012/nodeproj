# Node v1179 signed approval artifact draft authoring readiness explanation

Node v1179 adds the redaction policy authoring requirement for the controlled read-only shard preview signed approval chain.

## What changed

- Authoring requirement: SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_AUTHORING_READINESS_REDACTION_POLICY
- Authoring blocker: SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_AUTHORING_READINESS_REDACTION_POLICY_BLOCKER
- Category: policy
- Source binding: redaction policy package slot and guard from Node v1161 review package preflight.
- Purpose: keeps redaction policy bound to authoring readiness and prevents raw secret leakage.
