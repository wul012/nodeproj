# Node v1291 signed approval artifact draft text package comparison preflight

## What changed

Node v1291 contributes to the v1287-v1311 comparison preflight by covering the digest slice: package digest. It turns the corresponding v1286 submission slot into an offline comparison lane and pairs it with an acceptance control for uncompared or unacceptable package material.
...
The digest slice adds the first digest recheck lane so manual package material can later be compared without changing sibling state.
