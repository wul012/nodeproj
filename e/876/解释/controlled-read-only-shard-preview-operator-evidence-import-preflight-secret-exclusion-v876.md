# v876 Controlled read-only shard preview operator evidence import preflight secret exclusion

v876 adds `IMPORT_PREFLIGHT_SECRET_EXCLUSION` to the operator evidence import preflight chain. It consumes `ENTRY_WORKSHEET_SECRET_EXCLUSION` from the v861 manual evidence entry worksheet and keeps secret exclusion explicit in every future import slot.
...
No screenshot is needed because v876 adds service artifacts and Markdown route content without a new browser page.
