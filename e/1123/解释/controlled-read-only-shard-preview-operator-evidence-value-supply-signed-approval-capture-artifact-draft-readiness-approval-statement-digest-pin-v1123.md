# Node v1123 controlled read-only shard preview signed approval artifact draft readiness: approval statement digest pin lane

Node v1123 owns the future approval statement digest placeholder slice of the v1112-v1136 signed approval artifact draft readiness package. This is not a small archive-only marker: it adds a route-visible readiness lane, a matching readiness control, source draft preflight bindings, blocker semantics, gate coverage, renderer output, profile output, and focused test expectations.

## Functional Change

- Lane code: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_READINESS_APPROVAL_STATEMENT_DIGEST_PIN`.
- Lane kind: digest.
- Source v1111 draft preflight field: `approvalStatementDigest`.
- Control blocker: `REJECT_DRAFT_READINESS_APPROVAL_STATEMENT_DIGEST_UNPINNED`.
