# v859 Controlled read-only shard preview manual evidence entry worksheet policy/archive targets

v859 adds `ENTRY_WORKSHEET_POLICY_ARCHIVE_TARGETS` to the manual evidence entry worksheet chain. It consumes `INTAKE_REVIEW_POLICY_ARCHIVE_TARGETS` from the v836 evidence intake review package and adds policy and archive evidence status fields without importing runtime payload.
...
No screenshot is needed because v859 adds service artifacts and Markdown route content without a new browser page.
