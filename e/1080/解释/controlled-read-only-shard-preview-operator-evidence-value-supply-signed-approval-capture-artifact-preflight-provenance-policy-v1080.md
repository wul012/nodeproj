# Node v1080 signed approval capture artifact preflight provenance policy fragment

This archive records Node v1080 in the controlled read-only shard preview operator evidence value supply signed approval capture artifact preflight chain.

## Scope

- Adds or verifies the provenance policy fragment part of the signed approval capture artifact preflight surface.
- Binds artifact fragment `SIGNED_APPROVAL_CAPTURE_ARTIFACT_PREFLIGHT_PROVENANCE_POLICY` to source capture input `SIGNED_APPROVAL_CAPTURE_PREFLIGHT_PROVENANCE_POLICY` and required capture input `provenancePolicy`.
- Binds artifact seal `ARTIFACT_PREFLIGHT_PROVENANCE_POLICY_SEAL` to blocker `ARTIFACT_PREFLIGHT_PROVENANCE_POLICY_MISSING` so a future artifact draft cannot silently skip this requirement.
...
Java and mini-kv can continue in parallel. Node v1080 does not require fresh sibling evidence and is not a pre-approval blocker for their next work.
