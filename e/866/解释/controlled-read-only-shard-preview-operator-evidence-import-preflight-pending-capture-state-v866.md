# v866 Controlled read-only shard preview operator evidence import preflight pending capture state

v866 adds `IMPORT_PREFLIGHT_PENDING_CAPTURE_STATE` to the operator evidence import preflight chain. It consumes `ENTRY_WORKSHEET_PENDING_CAPTURE_STATE` from the v861 manual evidence entry worksheet and keeps already-captured runtime evidence blocked at import preflight.
...
No screenshot is needed because v866 adds service artifacts and Markdown route content without a new browser page.
