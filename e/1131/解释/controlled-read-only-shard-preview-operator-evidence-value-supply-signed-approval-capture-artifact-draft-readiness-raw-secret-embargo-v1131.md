# Node v1131 controlled read-only shard preview signed approval artifact draft readiness: raw secret embargo lane

Node v1131 owns the raw secret and signature material embargo slice of the v1112-v1136 signed approval artifact draft readiness package. This is not a small archive-only marker: it adds a route-visible readiness lane, a matching readiness control, source draft preflight bindings, blocker semantics, gate coverage, renderer output, profile output, and focused test expectations.

## Functional Change

- Lane code: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_READINESS_RAW_SECRET_EMBARGO`.
- Lane kind: execution lock.
- Source v1111 draft preflight field: `noRawSecretLock`.
- Control blocker: `REJECT_DRAFT_READINESS_RAW_SECRET_PRESENT`.
