# Node v1135 controlled read-only shard preview signed approval artifact draft readiness: sibling non-mutation evidence lane

Node v1135 owns the Java and mini-kv non-mutation guarantee slice of the v1112-v1136 signed approval artifact draft readiness package. This is not a small archive-only marker: it adds a route-visible readiness lane, a matching readiness control, source draft preflight bindings, blocker semantics, gate coverage, renderer output, profile output, and focused test expectations.

## Functional Change

- Lane code: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_READINESS_SIBLING_NON_MUTATION_EVIDENCE`.
- Lane kind: execution lock.
- Source v1111 draft preflight field: `siblingNonMutationLock`.
- Control blocker: `REJECT_DRAFT_READINESS_SIBLING_MUTATION_ENABLED`.
