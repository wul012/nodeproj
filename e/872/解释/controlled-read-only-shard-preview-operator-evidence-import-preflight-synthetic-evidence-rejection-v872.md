# v872 Controlled read-only shard preview operator evidence import preflight synthetic evidence rejection

v872 adds `IMPORT_PREFLIGHT_SYNTHETIC_EVIDENCE_REJECTION` to the operator evidence import preflight chain. It consumes `ENTRY_WORKSHEET_SYNTHETIC_EVIDENCE_REJECTION` from the v861 manual evidence entry worksheet and blocks synthetic evidence acceptance before importer implementation.
...
No screenshot is needed because v872 adds service artifacts and Markdown route content without a new browser page.
