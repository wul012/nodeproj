# v898 Controlled read-only shard preview operator evidence value draft target coverage

v898 adds `VALUE_DRAFT_TARGET_COVERAGE` to the operator evidence value draft chain. It consumes `IMPORT_PREFLIGHT_TARGET_COVERAGE` from the v862-v886 import preflight and contributes Node, Java, and mini-kv target coverage draft counts.
...
- draft code: `VALUE_DRAFT_TARGET_COVERAGE`;
- source preflight slot: `IMPORT_PREFLIGHT_TARGET_COVERAGE`;
- focus: Node, Java, and mini-kv target coverage draft counts;
- boundary: does not start sibling services.
...
No screenshot is needed because v898 adds service artifacts and Markdown route content without a new browser page.
