# v896 Controlled read-only shard preview operator evidence value draft runtime payload exclusion

v896 adds `VALUE_DRAFT_RUNTIME_PAYLOAD_EXCLUSION` to the operator evidence value draft chain. It consumes `IMPORT_PREFLIGHT_RUNTIME_PAYLOAD_EXCLUSION` from the v862-v886 import preflight and contributes runtime payload exclusion draft flags.
...
- draft code: `VALUE_DRAFT_RUNTIME_PAYLOAD_EXCLUSION`;
- source preflight slot: `IMPORT_PREFLIGHT_RUNTIME_PAYLOAD_EXCLUSION`;
- focus: runtime payload exclusion draft flags;
- boundary: rejects pasted runtime payload text.
...
No screenshot is needed because v896 adds service artifacts and Markdown route content without a new browser page.
