# v897 Controlled read-only shard preview operator evidence value draft synthetic evidence rejection

v897 adds `VALUE_DRAFT_SYNTHETIC_EVIDENCE_REJECTION` to the operator evidence value draft chain. It consumes `IMPORT_PREFLIGHT_SYNTHETIC_EVIDENCE_REJECTION` from the v862-v886 import preflight and contributes synthetic evidence rejection draft flags.
...
- draft code: `VALUE_DRAFT_SYNTHETIC_EVIDENCE_REJECTION`;
- source preflight slot: `IMPORT_PREFLIGHT_SYNTHETIC_EVIDENCE_REJECTION`;
- focus: synthetic evidence rejection draft flags;
- boundary: keeps synthetic samples outside the package.
...
No screenshot is needed because v897 adds service artifacts and Markdown route content without a new browser page.
