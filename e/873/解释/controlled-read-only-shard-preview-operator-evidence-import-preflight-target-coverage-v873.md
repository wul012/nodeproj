# v873 Controlled read-only shard preview operator evidence import preflight target coverage

v873 adds `IMPORT_PREFLIGHT_TARGET_COVERAGE` to the operator evidence import preflight chain. It consumes `ENTRY_WORKSHEET_TARGET_COVERAGE` from the v861 manual evidence entry worksheet and preserves Node, Java, mini-kv, and policy/archive target coverage.
...
No screenshot is needed because v873 adds service artifacts and Markdown route content without a new browser page.
