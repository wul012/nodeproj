# v867 Controlled read-only shard preview operator evidence import preflight required fields

v867 adds `IMPORT_PREFLIGHT_REQUIRED_FIELDS` to the operator evidence import preflight chain. It consumes `ENTRY_WORKSHEET_REQUIRED_FIELDS` from the v861 manual evidence entry worksheet and preserves required field coverage before import can be considered.
...
No screenshot is needed because v867 adds service artifacts and Markdown route content without a new browser page.
