# v882 Controlled read-only shard preview operator evidence import preflight Java target

v882 adds `IMPORT_PREFLIGHT_JAVA_TARGET` to the operator evidence import preflight chain. It consumes `ENTRY_WORKSHEET_JAVA_TARGET` from the v861 manual evidence entry worksheet and defines Java status normalization while preventing Node from starting Java.
...
No screenshot is needed because v882 adds service artifacts and Markdown route content without a new browser page.
