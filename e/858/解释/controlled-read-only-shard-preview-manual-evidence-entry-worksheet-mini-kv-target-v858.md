# v858 Controlled read-only shard preview manual evidence entry worksheet mini-kv target

v858 adds `ENTRY_WORKSHEET_MINI_KV_TARGET` to the manual evidence entry worksheet chain. It consumes `INTAKE_REVIEW_MINI_KV_TARGET` from the v836 evidence intake review package and adds the mini-kv SHARDJSON status fields while keeping the command allowlist narrow.
...
No screenshot is needed because v858 adds service artifacts and Markdown route content without a new browser page.
