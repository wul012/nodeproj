# Node v1121 controlled read-only shard preview signed approval artifact draft readiness: detached signature placeholder review lane

Node v1121 owns the detached signature placeholder without payload slice of the v1112-v1136 signed approval artifact draft readiness package. This is not a small archive-only marker: it adds a route-visible readiness lane, a matching readiness control, source draft preflight bindings, blocker semantics, gate coverage, renderer output, profile output, and focused test expectations.

## Functional Change

- Lane code: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_READINESS_DETACHED_SIGNATURE_REVIEW`.
- Lane kind: signature.
- Source v1111 draft preflight field: `detachedSignaturePlaceholder`.
- Control blocker: `REJECT_DRAFT_READINESS_DETACHED_SIGNATURE_PLACEHOLDER_UNREVIEWED`.
