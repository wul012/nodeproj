# v894 Controlled read-only shard preview operator evidence value draft redaction rules

v894 adds `VALUE_DRAFT_REDACTION_RULES` to the operator evidence value draft chain. It consumes `IMPORT_PREFLIGHT_REDACTION_RULES` from the v862-v886 import preflight and contributes redaction coverage draft counts.
...
- draft code: `VALUE_DRAFT_REDACTION_RULES`;
- source preflight slot: `IMPORT_PREFLIGHT_REDACTION_RULES`;
- focus: redaction coverage draft counts;
- boundary: preserves the preflight redaction rule before values exist.
...
No screenshot is needed because v894 adds service artifacts and Markdown route content without a new browser page.
