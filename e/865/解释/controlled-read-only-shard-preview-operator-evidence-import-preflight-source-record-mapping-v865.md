# v865 Controlled read-only shard preview operator evidence import preflight source record mapping

v865 adds `IMPORT_PREFLIGHT_SOURCE_RECORD_MAPPING` to the operator evidence import preflight chain. It consumes `ENTRY_WORKSHEET_SOURCE_RECORD_MAPPING` from the v861 manual evidence entry worksheet and defines mapping count normalization and missing-mapping rejection.
...
No screenshot is needed because v865 adds service artifacts and Markdown route content without a new browser page.
