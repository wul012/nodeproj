# v845 Controlled read-only shard preview manual evidence entry worksheet manual input state

v845 adds `ENTRY_WORKSHEET_MANUAL_INPUT_STATE` to the manual evidence entry worksheet chain. It consumes `INTAKE_REVIEW_MANUAL_INPUT_STATE` from the v836 evidence intake review package and locks the worksheet to not-entered manual state instead of silently accepting values.
...
No screenshot is needed because v845 adds service artifacts and Markdown route content without a new browser page.
