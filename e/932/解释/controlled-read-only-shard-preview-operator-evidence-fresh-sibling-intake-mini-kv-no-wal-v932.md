# Node v932 controlled read-only shard preview operator evidence fresh sibling intake: mini-kv WAL boundary

## What This Version Owns

v932 contributes mini-kv WAL boundary to the Node v912-v936 fresh sibling intake package. The slice is owned by mini-kv evidence consumption, but the implementation lives only in Node and reads committed sibling files through historical fallback-aware evidence helpers.

Detail: matches walTouched:false and preserves no-write evidence boundaries.
