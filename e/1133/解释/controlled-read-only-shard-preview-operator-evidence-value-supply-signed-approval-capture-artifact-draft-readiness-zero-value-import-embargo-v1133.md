# Node v1133 controlled read-only shard preview signed approval artifact draft readiness: zero value import embargo lane

Node v1133 owns the zero value submission and import guarantee slice of the v1112-v1136 signed approval artifact draft readiness package. This is not a small archive-only marker: it adds a route-visible readiness lane, a matching readiness control, source draft preflight bindings, blocker semantics, gate coverage, renderer output, profile output, and focused test expectations.

## Functional Change

- Lane code: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_READINESS_ZERO_VALUE_IMPORT_EMBARGO`.
- Lane kind: execution lock.
- Source v1111 draft preflight field: `zeroValueImportLock`.
- Control blocker: `REJECT_DRAFT_READINESS_VALUE_IMPORT_NONZERO`.
