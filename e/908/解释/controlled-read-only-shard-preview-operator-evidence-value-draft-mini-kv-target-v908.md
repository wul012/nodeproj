# v908 Controlled read-only shard preview operator evidence value draft mini-kv target

v908 adds `VALUE_DRAFT_MINI_KV_TARGET` to the operator evidence value draft chain. It consumes `IMPORT_PREFLIGHT_MINI_KV_TARGET` from the v862-v886 import preflight and contributes mini-kv target status draft labels.
...
- draft code: `VALUE_DRAFT_MINI_KV_TARGET`;
- source preflight slot: `IMPORT_PREFLIGHT_MINI_KV_TARGET`;
- focus: mini-kv target status draft labels;
- boundary: keeps the command boundary at SHARDJSON.
...
No screenshot is needed because v908 adds service artifacts and Markdown route content without a new browser page.
