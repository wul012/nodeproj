# v883 Controlled read-only shard preview operator evidence import preflight mini-kv target

v883 adds `IMPORT_PREFLIGHT_MINI_KV_TARGET` to the operator evidence import preflight chain. It consumes `ENTRY_WORKSHEET_MINI_KV_TARGET` from the v861 manual evidence entry worksheet and defines mini-kv SHARDJSON status normalization and command guard.
...
No screenshot is needed because v883 adds service artifacts and Markdown route content without a new browser page.
