# v864 Controlled read-only shard preview operator evidence import preflight version order

v864 adds `IMPORT_PREFLIGHT_VERSION_ORDER` to the operator evidence import preflight chain. It consumes `ENTRY_WORKSHEET_VERSION_ORDER` from the v861 manual evidence entry worksheet and defines canonical version label normalization for the import preflight.
...
No screenshot is needed because v864 adds service artifacts and Markdown route content without a new browser page.
