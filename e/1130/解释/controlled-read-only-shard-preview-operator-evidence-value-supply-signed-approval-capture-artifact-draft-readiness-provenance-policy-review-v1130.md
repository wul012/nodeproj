# Node v1130 controlled read-only shard preview signed approval artifact draft readiness: provenance policy review lane

Node v1130 owns the provenance policy review slice of the v1112-v1136 signed approval artifact draft readiness package. This is not a small archive-only marker: it adds a route-visible readiness lane, a matching readiness control, source draft preflight bindings, blocker semantics, gate coverage, renderer output, profile output, and focused test expectations.

## Functional Change

- Lane code: `SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_READINESS_PROVENANCE_POLICY_REVIEW`.
- Lane kind: policy.
- Source v1111 draft preflight field: `provenancePolicy`.
- Control blocker: `REJECT_DRAFT_READINESS_PROVENANCE_POLICY_UNREVIEWED`.
