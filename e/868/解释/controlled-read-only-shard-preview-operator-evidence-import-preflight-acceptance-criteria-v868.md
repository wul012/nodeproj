# v868 Controlled read-only shard preview operator evidence import preflight acceptance criteria

v868 adds `IMPORT_PREFLIGHT_ACCEPTANCE_CRITERIA` to the operator evidence import preflight chain. It consumes `ENTRY_WORKSHEET_ACCEPTANCE_CRITERIA` from the v861 manual evidence entry worksheet and preserves acceptance criterion coverage for a later value importer.
...
No screenshot is needed because v868 adds service artifacts and Markdown route content without a new browser page.
