# v863 Controlled read-only shard preview operator evidence import preflight control count

v863 adds `IMPORT_PREFLIGHT_CONTROL_COUNT` to the operator evidence import preflight chain. It consumes `ENTRY_WORKSHEET_CONTROL_COUNT` from the v861 manual evidence entry worksheet and defines count normalization and mismatch rejection before any operator value is accepted.
...
No screenshot is needed because v863 adds service artifacts and Markdown route content without a new browser page.
