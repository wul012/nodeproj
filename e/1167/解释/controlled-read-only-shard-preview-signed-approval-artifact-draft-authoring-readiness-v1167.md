# Node v1167 signed approval artifact draft authoring readiness explanation

Node v1167 adds the operator role authoring requirement for the controlled read-only shard preview signed approval chain.

## What changed

- Authoring requirement: SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_AUTHORING_READINESS_OPERATOR_ROLE
- Authoring blocker: SIGNED_APPROVAL_CAPTURE_ARTIFACT_DRAFT_AUTHORING_READINESS_OPERATOR_ROLE_BLOCKER
- Category: identity
- Source binding: operator role package slot and guard from Node v1161 review package preflight.
- Purpose: exposes reviewed operator role and keeps the draft authoring path separate from approval capture.
