# v875 Controlled read-only shard preview operator evidence import preflight failure class coverage

v875 adds `IMPORT_PREFLIGHT_FAILURE_CLASS_COVERAGE` to the operator evidence import preflight chain. It consumes `ENTRY_WORKSHEET_FAILURE_CLASS_COVERAGE` from the v861 manual evidence entry worksheet and preserves failure class coverage without turning triage into execution.
...
No screenshot is needed because v875 adds service artifacts and Markdown route content without a new browser page.
