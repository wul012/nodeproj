# v906 Controlled read-only shard preview operator evidence value draft Node target

v906 adds `VALUE_DRAFT_NODE_TARGET` to the operator evidence value draft chain. It consumes `IMPORT_PREFLIGHT_NODE_TARGET` from the v862-v886 import preflight and contributes Node target status draft labels.
...
- draft code: `VALUE_DRAFT_NODE_TARGET`;
- source preflight slot: `IMPORT_PREFLIGHT_NODE_TARGET`;
- focus: Node target status draft labels;
- boundary: stores digest references rather than raw route responses.
...
No screenshot is needed because v906 adds service artifacts and Markdown route content without a new browser page.
