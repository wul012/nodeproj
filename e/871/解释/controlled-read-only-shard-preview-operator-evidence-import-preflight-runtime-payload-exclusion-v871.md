# v871 Controlled read-only shard preview operator evidence import preflight runtime payload exclusion

v871 adds `IMPORT_PREFLIGHT_RUNTIME_PAYLOAD_EXCLUSION` to the operator evidence import preflight chain. It consumes `ENTRY_WORKSHEET_RUNTIME_PAYLOAD_EXCLUSION` from the v861 manual evidence entry worksheet and blocks runtime payload import at the preflight boundary.
...
No screenshot is needed because v871 adds service artifacts and Markdown route content without a new browser page.
