...
- 禁止用编号列表重复同一段长句来满足字数。长讲解必须按不同代码路径、响应字段、安全边界、验证证据和维护取舍展开，让读者能跳读。
- 禁止把大部分内容塞进一个 `Detailed Walkthrough / 详细讲解` 或 `详细说明` 章节。长讲解应拆成可扫读的小节，例如阅读索引、模型说明、安全边界、失败排查、维护取舍和停止条件。
