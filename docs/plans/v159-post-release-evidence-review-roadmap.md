...
计划状态：已完成。上一份 `docs/plans/v156-post-read-only-window-capture-roadmap.md` 已完成，本文件从 Java v52 / mini-kv v61 / Node v160 开始，并在 Node v161 `controlled idempotency drill runbook` 收口。后续计划已另起 `docs/plans/v161-post-controlled-idempotency-drill-roadmap.md`。
...
Java v52 和 mini-kv v61 已经完成第一组推荐并行上游能力，Node v160 已经把两者汇总成只读纵向 readiness review。

Java v53 和 mini-kv v62 已经完成第二组推荐并行上游能力，Node v161 已经把两者合成 controlled idempotency drill runbook。当前幂等纵向切片已收口。
...
```text
1. 已完成：推荐并行 Java v52 + mini-kv v61。
...
2. 已完成：Node v160 idempotency vertical readiness review，已读取 Java v52 和 mini-kv v61 的版本证据，生成纵向切片 readiness review，默认不执行写操作。
3. 已完成：推荐并行 Java v53 + mini-kv v62。
...
4. 已完成：Node v161 controlled idempotency drill runbook，已读取 Java v53 和 mini-kv v62 的完成证据，生成人工演练 runbook，只读或 dry-run，不自动启动 Java / mini-kv。
5. 后续从 `docs/plans/v161-post-controlled-idempotency-drill-roadmap.md` 接力。
```

## 并行依赖说明

```text
Java v52 与 mini-kv v61 推荐并行，因为两者都在各自项目内部落能力，不互相调用；该阶段已完成。
Node v160 已完成，它消费 Java v52 和 mini-kv v61 的完成证据，不执行写操作。
Java v53 与 mini-kv v62 推荐并行，因为 Java 只抽象 disabled candidate，mini-kv 只补 TTL recovery evidence；该阶段已完成。
Node v161 已完成，它消费 Java v53 和 mini-kv v62 的完成证据，不执行写操作。
```
