...
下一步：
当前下一步是 Node v230 manual sandbox connection preflight gate。
v230 仍只做连接前 gate，不打开 managed audit 连接，不读取 credential value，不启动 Java / mini-kv / 外部审计服务。
...
Node v230 前置项：
- 必须消费 Node v229 verification，不绕过 Java v87 / mini-kv v96 证据。
- 只生成 preflight gate，不打开连接、不读 credential value、不启动上游。
- 若需要 Java v88 / mini-kv v97 新证据，应写成推荐并行，不把两边误写成串行阻塞。
```
...
v229-post-packet-verification-roadmap.md
 -> 当前唯一有效全局计划；由 Node v229 manual sandbox connection packet verification 衍生；下一步 Node v230 manual sandbox connection preflight gate，仍不打开 managed audit 连接
```
