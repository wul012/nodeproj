...
Node v319 已完成 credential-handle approval prerequisite closure review；当前完成 3/6 prerequisite，剩余 endpoint-handle-allowlist-approval、no-network-safety-fixture、abort-rollback-semantics。
下一步：Node v320 做 endpoint-handle allowlist approval contract intake，仍不解析 raw endpoint URL、不打开 managed audit connection。
