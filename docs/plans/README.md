...
Node v320 已完成 endpoint-handle allowlist approval contract intake；当前完成 3/6 prerequisite，剩余 endpoint-handle-allowlist-approval、no-network-safety-fixture、abort-rollback-semantics。
下一步：推荐并行 Java v147 + mini-kv v140，只读 echo / non-participation，仍不解析 raw endpoint URL、不打开 managed audit connection。
