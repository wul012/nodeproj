...
下一步：进入 `v252-post-disabled-adapter-client-precheck-roadmap.md`。Node v253 可以做 test-only adapter shell contract；随后推荐并行 Java v102 + mini-kv v111 做只读 echo / non-participation，再由 Node v254 做三方 verification。
...
v252-post-disabled-adapter-client-precheck-roadmap.md
 -> 当前唯一有效全局计划；由 Node v252 disabled adapter client precheck 衍生；下一步 Node v253 test-only adapter shell contract，随后推荐并行 Java v102 + mini-kv v111，只读回显/非参与后由 Node v254 做三方验证
```
