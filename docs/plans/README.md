...
Java v99 只读回显 Node v245 precheck packet；mini-kv v108 只读证明 precheck packet 不会导致 auto-start / write / credential read / restore / storage backend。两边完成前，Node v246 必须停止，不得抢跑。
...
v245-post-sandbox-precheck-roadmap.md
 -> 当前唯一有效全局计划；由 Node v245 sandbox connection precheck packet 衍生；当前下一步推荐并行 Java v99 + mini-kv v108，再由 Node v246/v247 做 precheck receipt verification 与 rehearsal guard
```
