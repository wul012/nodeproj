...
```text
1. Node v177：CI operator identity evidence packet，消费 v176，固化 CI / local smoke 的 operator identity、role、auth-source 证据，不接真实 IdP。
2. 推荐并行：Java v63 + mini-kv v72。
...
- 引用 Node v176 packet digest。
- 记录 local smoke header identity：operator id、roles、auth source。
- 记录 GitHub Actions 的 actor / workflow identity 期望字段，但不读取生产 secret。
