...
5. 推荐并行：Java v67 + mini-kv v76，下一步执行。
...
6. Node v190：real HTTP read adapter rehearsal。
...
依赖关系：等待 Node v188 step/validation split 完成后再并行推进。理由是 v186+v187 后 `opsPromotionArchiveBundle.ts` 仍约 6966 行，超过 5000 行，继续拆分最大技术债比立刻进入 real HTTP 更稳。
...
```text
opsPromotionArchiveBundle.ts 已降到约 3743 行，进入 3000-4000 行区间；下一步可进入 Java v67 + mini-kv v76 并行，再进入 Node v190 real HTTP read adapter rehearsal。
```
...
## Node v190：real HTTP read adapter rehearsal

依赖关系：等待 Node v189、Java v67 + mini-kv v76 完成。
...
```text
v185 之后不要继续堆横向证据；先用 Node v186-v189 加速拆分最大技术债，再让 Java v67 + mini-kv v76 学习 Java v66 的真实只读能力节奏，最后由 Node v190 进入可关闭的真实读取 adapter rehearsal。
```
