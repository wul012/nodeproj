...
### Node v65：Upstream approval evidence integration

目标：

```text
统一接入 Java v40 failed-event approval-status 与 mini-kv v49 EXPLAINJSON side_effects，
让 approval evidence report 同时包含本地审批证据和新增上游只读证据。
```
...
- 不新增真实 replay / approval POST
- 不直接执行 mini-kv 写命令
- 不修改 Java / mini-kv 项目
- 不把 Java v40 或 mini-kv v49 的缺失当成 Node 可绕过的理由

Node v65 实施收口：

```text
已按本节把 Java approval-status 与 mini-kv side_effects 接入 approval evidence report / verification。
本版保持 upstream action 关闭时仍可只读收集证据，仍不新增真实执行入口。
后续计划转入 docs/plans/v65-post-upstream-evidence-roadmap.md。
```
...
8. Node v66 及之后：转入 docs/plans/v65-post-upstream-evidence-roadmap.md
```
