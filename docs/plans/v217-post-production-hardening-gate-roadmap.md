...
```text
1. Node v218：audit route + managed audit helper quality pass。
...
```text
Node v218 把 auditRoutes helper 和 managedAudit* helper 收口放在同一版，因为它们同属 Node 路由/报告重复结构治理，且不依赖上游。
...
- managedAudit* readiness gate / archive verification helper 抽 report metadata、message rules、endpoint constants，避免单文件继续膨胀。
- 若改动超过合理小闭环，优先完成 route registrar，helper 拆分留到 v219 或 v220。
