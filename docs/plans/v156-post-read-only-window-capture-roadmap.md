...
计划状态：已完成；Node v157-v159 已完成，Java v51 + mini-kv v60 已只读核对完成。后续从 `docs/plans/v159-post-release-evidence-review-roadmap.md` 接力。
...
3. Java v51 + mini-kv v60：已一起完成，只补只读 capture 后的证据解释增强；Java 补 ops evidence 字段说明，mini-kv 补 INFOJSON/STATSJSON 字段说明。
4. Node v159：已完成 read-only capture release evidence review，把 v157/v158 和上游 v51/v60 说明合成一次 release evidence review；仍保持 production operations=false。
```
...
状态：已由 Java v51 和 mini-kv v60 完成。
...
```text
Node v157-v159 已完成只读 capture archive、verification、release evidence review；下一阶段另起 plan，开始选择真正纵向深推进的生产硬化切片。
```
