...
2. Node v164：cross-project release bundle gate，必须等待 Java v56 和 mini-kv v65 都完成后再做；合成三项目 release bundle gate，只读，不启动上游。
3. 推荐并行：Java v57 + mini-kv v66。
...
4. Node v165：rollback window readiness checklist，必须等待 Java v57 和 mini-kv v66 都完成后再做；合成跨项目人工窗口 checklist，仍然 dry-run。
```
...
Node v164 不能提前做，因为它要消费 Java v56 和 mini-kv v65 的完成证据。
Java v57 与 mini-kv v66 推荐并行，因为两者都只补人工 handoff sample，不执行真实回滚。
Node v165 不能提前做，因为它要消费 Java v57 和 mini-kv v66 的完成证据。
```
...
## Node v164：cross-project release bundle gate
...
## Node v165：rollback window readiness checklist
