...
计划状态：已完成并收口；由 `docs/plans/v166-post-rollback-window-roadmap.md` 接续。上一份 `docs/plans/v161-post-controlled-idempotency-drill-roadmap.md` 到 Node v163 收口。
...
2. 已完成推荐并行：Java v56 + mini-kv v65。
...
3. 已完成：Node v165。
   Node v165 做 cross-project release bundle gate，消费 Java v56 和 mini-kv v65 的 bundle 证据；只读，不启动上游，不授权生产发布或回滚。
4. 已完成推荐并行：Java v57 + mini-kv v66。
...
5. 已完成：Node v166。
   Node v166 做 rollback window readiness checklist，消费 Java v57 和 mini-kv v66 的 handoff 证据；只读，不启动上游，不授权真实回滚或恢复。
6. 后续由 `docs/plans/v166-post-rollback-window-roadmap.md` 接续，不继续在本计划追加新版本。
```
...
Node v165 已在 Java v56 和 mini-kv v65 完成后推进完成，消费两边 release bundle evidence 但不执行上游动作。
Java v57 与 mini-kv v66 已推荐并行完成，因为两者都只补人工 handoff sample，不执行真实回滚。
Node v166 已在 Java v57 和 mini-kv v66 完成后推进完成，消费两边 handoff evidence 但不执行上游动作。
```
...
```text
Node v164
Java v56 + mini-kv v65
Node v165
Java v57 + mini-kv v66
Node v166
```

后续不继续追加到本文件，由 `docs/plans/v166-post-rollback-window-roadmap.md` 接续。
