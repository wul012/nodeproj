...
计划状态：当前唯一有效全局计划。v225 已消费 Node v224、Java v82、mini-kv v91/v94，生成 sandbox adapter dry-run package，并融合 auditRoutes 旧 JSON/Markdown 路由迁移。下一阶段仍不能直接连接生产 managed audit；如果要靠近真实联调，应先做 manual sandbox connection runbook 和只读/禁写 guard。
...
```text
1. Node v226：manual sandbox adapter connection runbook。
...
3. Node v227：manual sandbox connection evidence checklist。
...
```text
Node v226 是 runbook，不应和 Node v227 checklist 合并。
Java v86 + mini-kv v95 推荐并行，因为二者都是只读 guard，不互相依赖。
...
```text
v225 已具备 sandbox dry-run package；下一阶段先做人工 sandbox connection runbook 和只读 guard，不能跳到自动连接或生产审计。
```
