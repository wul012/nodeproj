...
```text
已按本节新增 execution gate preview：
基于 approval evidence handoff bundle 判断 request、decision、verification、上游 digest/schema 证据和 upstream untouched 是否满足执行前复核条件。
本版仍只返回 previewOnly=true、executionAllowed=false，不调用 Java replay POST，不执行 mini-kv 写命令。
```
...
4. Node v68：execution gate preview，已完成，仍不真实执行
```
