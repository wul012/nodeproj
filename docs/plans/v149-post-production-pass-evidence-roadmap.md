...
计划状态：当前有效全局计划；Java v49 + mini-kv v58 已完成，当前推进 Node v152。
...
2. Node v152：production pass evidence archive verification，消费 Node v150 archive，并预留 Java v49 / mini-kv v58 evidence 引用位；若上游尚未完成，明确标记 upstreamEvidenceReady=false
...
Node v152：已具备上游版本证据，可推进 archive verification。
```
...
```text
v150-v152 已由 Node 完成 archive、共享工具收口和 archive verification；Java v49 + mini-kv v58 已提供只读 evidence。下一步是 Node v153 operator runbook。
```
