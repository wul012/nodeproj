...
3. Node v215：managed audit dry-run adapter candidate。
...
- managed audit dry-run adapter 的公共 result formatter / digest helper 抽取，避免 adapter candidate 文件过大。
```
...
```text
v213 已经把 restore drill plan 做出来，下一步先确认这份证据可归档、可复查，再用 Java/mini-kv 的只读边界回执约束 Node 的第一版 dry-run adapter candidate。
```
