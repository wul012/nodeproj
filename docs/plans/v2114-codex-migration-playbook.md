...
| 35 | v2153 | 8 profile-section fragment renderers | standardized the controlled read-only shard preview profile-section fragment renderers that v2152 intentionally deferred. Added `renderProfileEntrySections` to the existing `liveProbeReportUtils` helper layer instead of adding a new service file, kept three composition-only profile-section renderers unchanged, and preserved the full-report `verificationReportBuilder` boundary. Full-report census remains 158 builder-backed / 87 unmigrated because this batch targets local `string[]` profile fragments rather than complete reports | pre/post SHA-256 and length comparison 6/6 byte-identical including the live read-only window aggregate; `npx vitest run` 6 focused files / 7 tests; `npm run typecheck`; ratchet 1 file / 2 tests; `npm run lint` 0 errors / 263 existing warnings; code walkthrough quality gate 62/62 | ready in this commit |
| 36 | v2154 | 5 production read-only-window MarkdownRenderer tail reports | standardized the production live-probe real-read smoke read-only-window Markdown renderer family through `releaseReportShared`: readiness packet, live capture, capture archive, capture archive verification, and capture release evidence review. Added reusable release-report section helpers for header, entries, line lists, repeated item sections, artifact subsections, messages, and evidence/next-action tails. This is deliberately not counted as lowering the `verificationReportBuilder` census because these are release/probe report renderers rather than verification-report spec renderers | fixed-profile pre/post SHA-256 and length comparison 5/5 byte-identical; `npx vitest run` 5 focused files / 22 tests; `npm run typecheck`; ratchet 1 file / 2 tests; `npm run lint` 0 errors / 263 existing warnings; code walkthrough quality gate 63/63 | ready in this commit |
| 37 | v2156 | 11 disabled runtime shell design/draft full-report renderers | standardized the disabled runtime shell pre-plan/design/upstream/candidate-gate and design-draft/body-draft report family through `releaseReportShared` while keeping local gate/question/catalog/body/evidence line formatters in-place. This batch also records the census deviation: the old 87 figure was verificationReportBuilder-only; the broader standardized scan counts `verificationReportBuilder`, `releaseReportShared`, and `renderProfileEntrySections`, moving from 171/245 standardized before this batch to 182/245 after it, with 63 unstandardized renderer files remaining | archived-profile pre/post SHA-256 and length comparison 11/11 byte-identical; `npx vitest run` 11 focused files / 44 tests; `npm run typecheck`; ratchet 1 file / 2 tests; `npm run lint` 0 errors / existing warnings; full `npx vitest run --maxWorkers=4` 528 files / 1631 tests; `npm run build`; code walkthrough added | ready in this commit |
