...
Node v233：
- managed audit manual sandbox connection rehearsal packet review ready
- markerSpan=Node v232 + Java v89 + mini-kv v98
- 消费 Node v232 guard 类型聚合、Java v89 ContextHeaderField、mini-kv v98 execute_with_wal 证据
...
Java v89：
- ContextHeaderField record 组合优化已完成
- 收敛 header value/source/echoed 构造
- 不新增 approval 行为，不写 ledger，不执行 SQL，不读取 credential value，不打开 managed audit connection
...
```text
1. Node v232 + Java v89 + mini-kv v98 已完成。
...
- v233 已新增 rehearsal packet review endpoint，必须保持 review-only；如果 mini-kv runtime fixture 字段层级变化，优先按真实 fixture 结构修 Node 消费端，不修改上游证据含义。
