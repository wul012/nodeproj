...
Java v61 + mini-kv v70：已推荐并行完成，Node v173 可以消费人工审批/恢复演练证据
...
5. 已完成推荐并行：Java v61 + mini-kv v70。
...
6. Node v173：release window readiness packet，已完成；消费 Java v61 + mini-kv v70 和 Node v172，形成发布窗口人工 review 包，同时完成轻量共享 blocker helper 收敛。
7. 下一阶段：另起新 plan，不继续在本文件追加重合版本；优先安排生产级收口中的真实部署前 dry-run envelope、release review handoff 或 CI evidence hardening。
```
...
Node v173 必须等待 Java v61 + mini-kv v70 + Node v172 完成后再做，当前已完成。
v173 后必须另起新 plan，避免和本文件的已完成版本重复。
```
