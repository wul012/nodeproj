...
```text
1. Node v320：endpoint-handle allowlist approval contract intake。当前下一步。
...
```text
credential-handle-approval 已完成 contract + upstream echo + closure review 闭环；下一步是 Node v320 定义 endpoint-handle allowlist approval 的非 raw URL contract，完成后再推荐并行 Java v147 + mini-kv v140。
```
