...
计划状态：已完成。上一份 `docs/plans2/v319-post-credential-handle-prerequisite-closure-roadmap.md` 已完成 Node v320、Java v147、mini-kv v140、Node v321、Node v322 的 endpoint-handle-allowlist-approval 闭环；本计划另起续写，不回填旧版本，不写重复版本。v325 完成后，当前有效计划切换到 `docs/plans2/v325-post-no-network-safety-fixture-prerequisite-closure-roadmap.md`。
...
Node v325：no-network-safety-fixture prerequisite closure review 已完成；当前完成 5/6 prerequisite，剩余 1/6。
...
2. 推荐并行：Java v149 + mini-kv v141。已完成。
   - Node v323 完成后执行。
...
4. Node v325：no-network-safety-fixture prerequisite closure review。已完成。
   - 只有 Node v324 完成后再做。
...
5. 计划续写：`docs/plans2/v325-post-no-network-safety-fixture-prerequisite-closure-roadmap.md`。当前下一步。
   - 先由 Node v326 定义 abort/rollback semantics contract intake。
   - 之后再推荐并行 Java v150 + mini-kv v142，只读 echo / non-participation。
```
...
Node v325 已完成 no-network-safety-fixture closure review，当前只剩 abort-rollback-semantics 一个 prerequisite；下一步切换到 `docs/plans2/v325-post-no-network-safety-fixture-prerequisite-closure-roadmap.md`，先做 Node v326 abort/rollback semantics contract intake。
```
