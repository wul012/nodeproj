...
```text
1. Node v323：no-network safety fixture contract intake。
...
3. Node v324：no-network safety fixture upstream echo verification。
...
4. Node v325：no-network-safety-fixture prerequisite closure review。
   - 只有 Node v324 完成后再做。
...
```text
endpoint-handle-allowlist-approval 已完成 contract + upstream echo + closure review 闭环；下一阶段只推进 no-network-safety-fixture，先由 Node v323 定义不可联网安全 fixture contract，再推荐并行 Java v149 + mini-kv v141，只读 echo / non-participation，最后由 Node v324/v325 验证并闭环。
```
