...
2. 推荐并行：Java v151 + mini-kv v143。当前下一步。
   - 只能在 Node v329 完成后执行。
...
3. Node v330：candidate gate upstream alignment / hardening review。等待 Java v151 + mini-kv v143 完成后再推进。
...
```text
6/6 prerequisite 已在 Node v328 闭合；Node v329 已完成 implementation candidate gate / input-hardening decision，并明确这仍不是 runtime shell 实施许可。下一步推荐并行 Java v151 + mini-kv v143，只做只读 input-hardening echo / receipt；Node v330 必须等待两边完成后再消费。
```
