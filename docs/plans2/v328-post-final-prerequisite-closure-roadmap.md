...
```text
1. Node v329：implementation candidate gate / input-hardening decision。
...
3. Node v330：candidate gate upstream alignment / hardening review。
   - 消费 Node v329 + Java v151 + mini-kv v143。
   - 判断下一步是继续 input export hardening，还是进入 disabled implementation design draft。
...
```text
6/6 prerequisite 已在 Node v328 闭合，但这只是 candidate gate 的前置条件，不是 runtime shell 实施许可。下一步 Node v329 应做 implementation candidate gate / input-hardening decision；Java v151 + mini-kv v143 只有在 v329 后才推荐并行只读 echo / receipt。
```
