...
```text
1. Node v347：minimal read-only integration smoke archive verification。当前下一步。
...
2. 推荐并行：Java v153 + mini-kv v144（仅当 Node v347 证明 v346 是 invalid-read-contract）。
...
3. Node v348：minimal read-only integration rerun decision。
...
```text
Node v346 已把最小只读联调推进到真实探测证据；下一步 Node v347 先归档验证这份证据，再决定是否需要 Java / mini-kv 补只读字段或等待外部窗口重试。
```
