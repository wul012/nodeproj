...
2. Node v293：fake harness readiness blocked decision upstream echo verification。
   ...
   - 执行前先做必要性证明和体量预估；若预估超过 3000 changed lines，拆成 catalog helper / verification report 两步。
   - 若需要新增第 8、9 段 echo，必须优先使用 catalog builder，不再复制完整长段逻辑。
...
- Java 后续 echo builder 可继续 catalog/template 化，把剩余 600+ builder 纳入统一模式；这是 Java 自己的质量优化，不阻塞 Node v293，除非 v293 明确依赖 Java 输出。
