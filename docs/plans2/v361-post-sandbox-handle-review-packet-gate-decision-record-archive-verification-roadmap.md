# Node v361 衍生计划：sandbox handle review packet/gate decision record archive verification 之后

来源版本：Node v361 `sandbox handle review packet/gate decision record archive verification`。

计划状态：v360 衍生计划中的 Node v361 已完成后另起续写；不回填旧计划，不写重复版本。
...
Java v153 + mini-kv v144：继续跳过。v349-v361 都没有 invalid-read-contract，因此不要求两边补字段。
...
```text
1. Node v362：sandbox handle review prerequisite closure review。当前下一步。
   - 消费 Node v361 archive verification。
   - 只审查是否可进入 sandbox handle review prerequisite closure 的下一阶段。
   - 可以引用 v360 decision=advance-to-sandbox-handle-review-prerequisite-closure-review。
   - 仍不能读取 credential value、raw endpoint URL、provider/client、runtime shell 或 managed audit HTTP/TCP。
   - 不启动 Java / mini-kv，不请求两边新版本。

2. Node v363 或后续：
   - 如果 v362 只是 closure review 且仍然只读，可以再归档验证 v362。
   - 如果 v362 发现缺截图、解释、讲解、route、Markdown、digest 或 summary，先修 archive verification，不推进功能。
...
```text
Node：
- v362 不复制 v361 archive reader；只消费 v361 profile 或 v361 archive evidence。
- 新增 service 继续分层 types/service/renderer/test；单 service 接近 700 行时先拆 helper/catalog。
- 测试继续分批：focused -> 小组 -> build -> smoke，不一次性跑大批量测试。
- 如果后续 closure review 开始重复 v354-v361 的边界字段，优先抽 shared boundary assertion/helper，再写功能。
...
```text
Node v361 已证明 v360 decision record 归档完整；下一步只能进入 Node v362 的 prerequisite closure review，不能把 archive verification 变成真实 credential、endpoint、provider/client、runtime shell 或 managed audit 连接实现。
```
