# Node v332 衍生计划：disabled design draft candidate archive verification 之后

来源版本：Node v332 `disabled design draft candidate archive verification`。

计划状态：v330 衍生计划中的 Node v331、Node v332 已形成闭环后另起续写；不回填旧计划，不写重复版本。
...
3. 后续是否推荐并行 Java v153 + mini-kv v144，由 Node v333/v334 的结论决定。
   - 如果只是 Node 自己的 outline intake / archive verification，不需要两边跟进。
   - 如果 outline intake 定义新的非 secret handoff 字段，才写成“推荐并行 Java v153 + mini-kv v144”。
```
...
```text
Node：
- v333/v334 继续沿用 types / service / renderer / test 拆分，禁止新增巨型 service。
- v333 如果 outline 字段开始膨胀，必须抽 section catalog / boundary catalog，不允许复制 700+ 行 service。
- 新增 governance/design 链必须写 necessity proof：解决哪个 blocker、谁消费、为什么不能复用 v332、何时停止。
- 单个 commit 目标低于 3000 changed lines；如超出，拆成代码、归档/截图、计划文档多个小提交。
- 归档验证要继续兼容 UTF-8 BOM 和 historical fallback。

Java：
- 当前不需要新的 Java 版本配合 Node v333，除非 v333 明确提出新的非 secret contract 字段。
- 若 Java 自行优化，仍优先 service/catalog 化，不把 echo builder 重新膨胀成 600+ 行模板。

mini-kv：
- 当前不需要新的 mini-kv 版本配合 Node v333，除非 v333 明确提出新的 non-participation receipt 字段。
...
- 如果 Node v333 不能说明 outline intake 的必要性、消费者和停止条件，必须暂停。
