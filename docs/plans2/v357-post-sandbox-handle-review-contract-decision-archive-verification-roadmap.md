...
Java v153 + mini-kv v144：继续跳过。v349-v357 都没有 invalid-read-contract，因此不要求两边补字段。
...
```text
1. Node v358：sandbox handle review packet / gate non-secret intake。当前下一步。
...
```text
Node v357 已证明 v356 contract decision 归档完整。下一步 Node v358 可以做 sandbox handle review packet/gate 的非 secret intake，但不能把 intake 变成真实 credential、endpoint、provider/client、runtime shell 或 managed audit 连接实现。
```
