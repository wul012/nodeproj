...
计划状态：当前有效全局计划。上一份 `docs/plans2/v305-post-stop-prerequisite-upstream-echo-roadmap.md` 已完成 Node v306、推荐并行 Java v142 + mini-kv v135、Node v307；后续不再向旧计划追加重合版本。
...
2. Node v308：human approval artifact review packet。
...
4. Node v309：human approval artifact review upstream echo verification。
...
Java：
- Java v143 如被 Node v308 请求，只做只读 echo，可继续 catalog/template 化，但不要把大型质量重构和 echo 混成不可审计版本。

mini-kv：
- mini-kv v136 如被 Node v308 请求，只做只读 non-participation receipt；不新增写命令，不做恢复/compact，不承载权威状态。

并行规则：
- 同一项目内部版本必须串行。
- Java v143 与 mini-kv v136 如被 Node v308 请求，互不依赖，推荐并行。
- Node v309 必须等待 Java v143 + mini-kv v136 完成，不能抢跑。
```
...
- Node v308 未完成时，Java v143 / mini-kv v136 不应提前声称消费 v308。

## 一句话结论

```text
v307 已完成三方 artifact contract 对齐；下一步 Node v308 应定义人工 approval artifact review packet，而不是直接实现 runtime shell。
```
