...
当前有效计划已切换到 docs/plans2/v313-post-prerequisite-catalog-cleanup-roadmap.md；下一步是 Node v314 signed human approval artifact contract intake。
...
Node v314：signed human approval artifact contract intake。当前下一步；先定义非 secret signed approval artifact contract，不请求 Java/mini-kv。
推荐并行：Java v145 + mini-kv v138。仅在 Node v314 完成后执行；两边互不依赖，可以并行只读 echo / non-participation。
...
- Node v314 只能推进 signed-human-approval-artifact 的非 secret contract intake；不关闭其余 4 个 prerequisite，不打开 runtime shell。
