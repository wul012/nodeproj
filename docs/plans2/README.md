...
Node v336：disabled design draft body intake archive verification 已完成；验证 v335 归档、route、Markdown、digest、截图/讲解/计划索引和 historical fallback。
...
Node v341：disabled design draft body preparation plan 已完成；只规划未来 body draft 的章节和证据映射，不写正式 body draft、不实现 runtime。
...
- Node v320 已完成 endpoint-handle allowlist contract intake；Java v147 + mini-kv v140 已完成并行上游证据，Java v148 已完成质量优化，Node v321 已完成三方 upstream echo verification，Node v322 已完成 endpoint-handle-allowlist-approval prerequisite closure review；Node v323-v325 已完成 no-network-safety-fixture contract、上游 echo 与 closure review；Node v326 已完成 abort/rollback semantics contract intake；Java v150 + mini-kv v142 已完成；Node v327 已完成 read-only cross-project readiness runner；Node v328 已完成 final prerequisite closure review；Node v329 已完成 implementation candidate gate / input-hardening decision；Java v151 + mini-kv v143 已完成，Java v152 已补 echo；Node v330 已完成 candidate gate upstream hardening review；Node v331 已完成 disabled design draft candidate review；Node v332 已完成 archive verification；Node v333 已完成 outline intake；Node v334 已完成 outline archive verification；Node v335 已完成 body intake / readiness step；Node v336 已完成 body intake archive verification；Node v337 已完成 body candidate review；Node v338 已完成 body candidate archive verification；Node v339 已完成 pre-draft decision；Node v340 已完成 pre-draft decision archive verification；Node v341 已完成 body preparation plan；Node v342 已完成 body preparation plan archive verification；Node v343 已完成 body draft candidate；Node v344 已完成 body draft candidate archive verification；Node v345 已完成 minimal read-only integration window readiness cut，并判定无需 Java v153 + mini-kv v144；Node v346 已完成 minimal read-only integration smoke rehearsal；当前有效计划是 docs/plans2/v346-post-minimal-read-only-integration-smoke-rehearsal-roadmap.md，下一步 Node v347。
...
- Node v347 已完成 minimal read-only integration smoke archive verification；归档结论是 read-window-unavailable，不是 invalid-read-contract，因此不请求 Java v153 + mini-kv v144 改代码。当前有效计划切换到 docs/plans2/v347-post-minimal-read-only-integration-smoke-archive-verification-roadmap.md，下一步 Node v348 只做 rerun decision。
- Node v348 已完成 minimal read-only integration rerun decision；结论是 wait-for-external-read-window，下一步 Node v349 只能在用户启动 Java / mini-kv 后重跑现有 smoke lane，否则只归档 pending。
...
- Node v367 已完成 minimal read-only integration gate execution；用户已启动 Java / mini-kv 后，Node 真实只读访问 5 个目标，5/5 read targets passed，20/20 checks passed，且 startsJavaService=false、startsMiniKvService=false、executionAllowed=false。当前有效计划切换到 docs/plans2/v367-post-minimal-read-only-integration-gate-execution-roadmap.md，下一步 Node v368 做 v367 archive verification，不要求 Java / mini-kv 新版本。
- Node v368 已完成 minimal read-only gate execution archive verification；11/11 archive files present，42/42 checks passed，未重跑 Java / mini-kv probe。当前有效计划切换到 docs/plans3/v368-post-minimal-read-only-integration-gate-execution-archive-verification-roadmap.md，下一步 Node v369 做 operator/CI regular gate handoff + contract freeze，之后推荐 mini-kv shard readiness prototype 与 Java shard readiness echo 并行推进。
- 从 Node v368 起，计划继续写入 docs/plans3/，代码讲解继续写入 代码讲解记录_生产雏形阶段3/，截图和解释继续放 e/<版本>/。
```
