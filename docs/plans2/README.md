...
Node v306：approval prerequisite artifact intake plan。当前下一步；先定义 approval prerequisite artifact 输入契约，不实现 runtime shell。
推荐并行：Java v142 + mini-kv v135。只有 Node v306 完成后才推进。
Node v307：approval prerequisite artifact upstream echo verification。必须等待 Java v142 + mini-kv v135 完成。
```
...
- Node v306 不允许继续空转 echo；必须围绕 approval prerequisite artifact intake，写明 blocker、consumer、复用边界和停止条件。
- 推荐并行：Java v142 + mini-kv v135。只有 Node v306 完成后才推进。
