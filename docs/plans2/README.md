...
当前有效计划已切换到 docs/plans2/v316-post-signed-artifact-prerequisite-closure-roadmap.md；下一步是 Node v317 credential-handle approval contract intake。
...
Node v317：credential-handle approval contract intake。当前下一步；先由 Node 定义非 secret contract，不并行 Java/mini-kv。
推荐并行：Java v146 + mini-kv v139。必须等 Node v317 完成后再做；两边互不依赖，可以并行只读 echo / non-participation。
Node v318：credential-handle approval upstream echo verification。必须等待 Java v146 + mini-kv v139 完成。
```
...
- Node v317 先做 credential-handle approval contract intake；Java v146 + mini-kv v139 只有在 Node v317 完成后才推荐并行。
