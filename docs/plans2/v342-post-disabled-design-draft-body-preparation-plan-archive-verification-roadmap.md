...
Node v344：disabled design draft body draft candidate archive verification 已完成；验证 v343 route/Markdown/evidence digest/截图/讲解/计划索引，未打开 runtime。
...
```text
1. Node v343：disabled design draft body draft candidate。已完成。
...
2. Node v344：disabled design draft body draft candidate archive verification。已完成。
...
```text
Node v344 已完成受限 body draft candidate 的 archive verification；当前有效计划切换到 docs/plans2/v344-post-disabled-design-draft-body-draft-candidate-archive-verification-roadmap.md，下一步转向最小只读联调 readiness，仍不是 runtime shell 实现或真实 managed audit connection。
```
