...
2. Node v344：disabled design draft body draft candidate archive verification。当前下一步。
...
```text
Node v343 已完成受限 body draft candidate；下一步只允许 Node v344 做 archive verification，仍不是 runtime shell 实现或真实 managed audit connection。
```
