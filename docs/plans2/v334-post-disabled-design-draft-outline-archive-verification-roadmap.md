...
```text
1. Node v335：disabled design draft body intake / readiness step。当前下一步。
...
2. Node v336：disabled design draft body intake archive verification。
...
```text
Node v334 已把 v333 outline intake 的归档证据验稳；下一步只允许 Node v335 做 disabled design draft body intake / readiness step，仍不是 runtime shell 实现。
```
