...
2. Node v336：disabled design draft body intake archive verification。已完成。
   - 只有 Node v335 完成且仍无副作用时才推进。
   - 验证 v335 route/Markdown/evidence digest/截图/讲解/计划索引。
   - 仍不打开真实 runtime，不请求 Java/mini-kv 新 echo。

3. Node v337：disabled design draft body candidate review。已另起计划承接。
   - 只做 body candidate review，不写 body draft、不实现 runtime。
   - 仍不请求 Java/mini-kv；除非 candidate review 定义新的非 secret handoff 字段。

4. 后续是否推荐并行 Java v153 + mini-kv v144，由 Node v337 的结论决定。
...
```text
Node v336 已把 v335 body intake 的归档证据验稳；下一步切换到 docs/plans2/v336-post-disabled-design-draft-body-intake-archive-verification-roadmap.md，只允许 Node v337 做 body candidate review，仍不是 body draft 或 runtime shell 实现。
```
