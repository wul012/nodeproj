...
```text
1. 推荐并行：Java v132 + mini-kv v130。
   - Java v132：read-only disabled runtime shell handoff echo。
...
2. Node v296：disabled runtime shell upstream echo verification。
   - 只能在 Java v132 + mini-kv v130 都完成后推进。
...
Java：
- Java v132 可以和 mini-kv v130 并行。
- Java v132 只做 read-only echo，不做 resolver runtime，不写 approval ledger，不执行 SQL，不调用外部 managed audit。
...
并行规则：
- 同一项目内部版本必须串行。
- Java v132 与 mini-kv v130 互不依赖，推荐并行。
- Node v296 依赖 Java v132 + mini-kv v130，不能与二者并行抢跑。
```
...
- Java v132 或 mini-kv v130 未完成时，Node v296 必须停止。

## 一句话结论

```text
v295 已完成 design review：下一步不是 Node 抢跑 runtime，而是推荐并行 Java v132 + mini-kv v130；两边完成后 Node v296 再做 upstream echo verification。
```
