...
Node v346：minimal read-only integration smoke rehearsal 已完成；复用现有 Java/mini-kv client 做五个只读目标探测，不启动上游、不写上游。
...
```text
1. Node v346：minimal read-only integration smoke rehearsal。已完成。
...
2. 推荐并行：Java v153 + mini-kv v144（仅当 Node v346 发现现有只读输出字段不足）。
   ...
   - 本轮是否需要执行这组并行版本，以 v346 归档 smokeState 为准：`invalid-read-contract` 才需要；`read-window-unavailable` 不需要改代码。

3. Node v347：minimal read-only integration smoke archive verification。当前下一步。
...
```text
Node v345 已确认最小只读联调字段足够；Node v346 已进入真实只读 smoke rehearsal。下一步 Node v347 归档验证 v346 smoke evidence，再决定是否需要 Java v153 / mini-kv v144 补只读字段。
```
