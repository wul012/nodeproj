...
```text
1. Node v345：minimal read-only integration window readiness cut。当前下一步。
   - 消费 Node v344。
...
2. 推荐并行：Java v153 + mini-kv v144（仅当 Node v345 发现缺少只读联调字段）。
   ...
   - 如果 Node v345 证明两边现有字段已经足够，则跳过这组并行版本。

3. Node v346：minimal read-only integration smoke rehearsal。
...
```text
Node v344 已把 v343 设计正文候选验成稳定归档；下一步 Node v345 开始收束到最小只读真实联调 readiness，而不是继续无限叠加 disabled design draft 层。
```
