# Node v353 衍生计划：managed-audit-disabled read-only integration decision record 之后

来源版本：Node v353 `managed-audit-disabled read-only integration decision record`。

计划状态：v352 衍生计划中的 Node v353 已完成 decision record 后另起续写；不回填旧计划，不写重复版本。
...
Java v153 + mini-kv v144：继续跳过。v349-v353 都没有 invalid-read-contract，因此不要求两边补字段。
...
```text
1. Node v354：sandbox handle review prerequisite intake。当前下一步。
   - 消费 Node v353 decision record。
   - 只定义非 secret 的 sandbox handle / allowlist review status 输入。
   - 明确 credential value、raw endpoint URL、provider/client、runtime shell、managed audit HTTP/TCP 仍关闭。
   - 不重新 live probe，不启动 Java / mini-kv。

2. Node v355 或后续：
   - 如果 v354 只是 prerequisite intake，可以做 archive verification。
   - 如果 v354 需要真实 managed audit sandbox credential、真实 endpoint handle 值、secret provider、resolver client 或 runtime shell，必须暂停并让用户确认授权与输入范围。
...
```text
Node：
- v354 只做 prerequisite intake，不复制 v353 decision 逻辑。
...
```text
Node v353 已把 v352 归档验证转成下一阶段决策。下一步 Node v354 可以定义 sandbox handle review prerequisite intake，但只能处理非 secret handle/review status，不得打开真实连接或执行能力。
```
