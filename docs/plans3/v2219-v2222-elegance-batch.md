...
2. `src/evidence/disabledAdapterEchoReferences`：字段规格拥有 JSON key、目标 key、类型与 fail-closed 默认值；
...
原 691 行服务已拆为 412 行 profile service 与 345 行 `src/evidence/disabledAdapterEchoReferences.ts`；后者唯一拥有
Java v102 / mini-kv v111 evidence、snippet、有序字段规格和 readiness。root-over-nested 的七个
兼容字段继续显式使用 `??`，旧 raw evidence metadata 由共享 helper 的窄 `textMode: "raw"` 选项
保留，默认 normalized 语义不变。首次 parity 失败定位并修复了 restore 字段的对象插入顺序，未修改
oracle。Windows raw 与跨平台 `<repo>` oracles、local/forced fallback、ready/blocked 路径全部保持，
focused 和文档门共 5 文件 22 项测试通过。维护性账本收紧为 84 / 99 / 220 / 0，准确删除一个
大文件、一个长函数和三个复杂函数；两个新职责文件均无替代热点。elegance 门曾阻止 `Evidence`
family 增长，模块按真实职责改名为 `Sources` 后通过，未放宽基线。讲解含 3,091 个中文字符、29 个
二级章节；临时 v2220 对比 worktree 与 junction 已删除，兄弟项目工作树未触碰。
...
原 740 行服务已拆为 468 行 profile service 与 335 行 `src/evidence/fakeTransportEchoReferences.ts`。v112 的 identity、
source、request、response 和 runtime 字段由五组有序规格读取，`-1` cleanup unknown 与危险侧 true
默认值均保持；Java/mini-kv readiness 和 19 项顶层 checks 分解为可命名的领域谓词。固定时间下，
Windows raw 与跨平台 `<repo>` 的 local、forced fallback、ready、blocked JSON/Markdown 摘要在迁移改动
前后逐字一致，迁移阶段测试期望和 fixture 未修改。维护性账本收紧为 83 / 98 / 217 / 0，准确删除一个
...
八个顺序分片在最多 4 workers 下最终覆盖 579 个唯一文件、1,764 项测试，零最终失败。第 2 分片首次
只触发 300 秒外层命令预算，未出现断言失败；PID 12104 专属测试树清理后以更合理的外层预算整片通过，
没有提高单测 timeout。第 7 分片首次由 governance ratchet 阻止 `src/services=1127`；两份 reference
迁入既有 `src/evidence` 后，治理门和整片均通过，硬上限仍为 1125/80。独立 `vitest list` 得到 579 个
文件、1,764 个唯一“文件+测试名”，重复为 0，与分片求和一致。

TypeScript build 通过。受保护 HTTP smoke 强制 historical fallback、关闭 probes/actions 并开启 access
guard：health 与零请求 metrics 通过；未匹配策略的 release readiness 按设计返回 403；v2222 profile
JSON/Markdown 返回 200、19/19 ready，execution 与 managed-audit connection 仍为 false。PID 22556、
43122 端口和 smoke 日志均已清理。远端 Node Evidence 首轮在四版统一推送后触发。

### CI 偏差与修复

GitHub Actions run `29801098105` 的 install、typecheck、lint、security、archive、elegance、family、renderer、
...
以及讲解质量门。完整 test/build/smoke/remote CI 在 v2222 执行；若该 run 失败，仅允许完成有证据的根因修复
后再复验一次。长验证期间只做下一版
...
- ratchet 放宽、任一债务计数增长、出现导入环，或在没有失败根因修复的情况下重复触发远端 CI。
