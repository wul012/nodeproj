...
| 三个批准输入模板由数据定义 | 模板定义与验证行为分离，目录 facade 稳定 | 模板顺序/键/摘要/目标路径、验证器和 live gate tests | v2218 local passed |
...
原 213 行 `buildApp` 已收缩为 10 行组合根；shell、请求 hook、运行时依赖和 15 个
有序路由阶段分别拥有单一职责。固定 `printRoutes` oracle 仍为 41,596 字节、460 行、
SHA-256 `e8fbc705761b2cf5f774d18e4879caf1b46ee6cb11046f4acdb71d6bbd778de9`。
最终定向矩阵通过 18 个文件、63 个测试，typecheck、定向 lint 和七项静态门全部通过；
维护性基线实际收紧为 85 / 106 / 226 / 0。未修改路由、响应、访问控制、fixture、
执行权限或兄弟项目文件，Java 与 mini-kv 可继续并行。
...
三个模板 case 已移入无运行时分支的有序 definitions，稳定目标路径由 4 行 constants 统一拥有；公开
catalog 保持导入路径与 re-export，并在每次创建前 fail-closed 校验精确数量、有序 key/owner/path、
archive path 分离、requiredFields、expectedConstants、semanticRules 与模板 identity。完整 JSON 仍为
7,304 字节和 SHA-256 `ead27560546b4dda1a640ec3141378573f5fd9cb395a6b617f7096cfcd85b08b`，
三个正文 digest 不变，多次读取的嵌套值相互隔离。原 216 行函数降为 3 行，基线实际收紧为
85 / 103 / 226 / 0；未创建批准输入、启动兄弟服务、修改 fixture 或打开 runtime gate。
批次级 full/build/smoke 已按约定在本版局部门完成后统一执行；远端 CI 等批次提交推送后只运行一次。

## 批次最终本地验收

- v2218 最终消费者矩阵：10 个文件、25 项测试；typecheck 与仓库级零警告 lint 通过。
- 静态门：security 8,322 文件、18/18；archive 64.26/80 MiB；elegance 158 families、
  52 tracked；family 52/52；renderer 242/245、3 waived、0 non-waived；source-size
  0 个文件超过 800 行；maintainability 85/103/226/0。
- 八个全量分片限制最多 4 workers，覆盖 578 个唯一文件、1,758 项测试，零失败、零重复；
  独立 discovery 得到相同文件与测试集合，双向遗漏和额外项均为 0。
- TypeScript build 通过。受保护 smoke 的 health、强制历史回退报告和受控预览均返回 200；
  三份 Java/mini-kv 证据全部来自仓库 historical fixtures，报告 30/30，preview 保持 blocked、
  execution=false、connectsManagedAudit=false。ops summary/readiness 也返回 200 且保持 blocked。
- 最终 smoke PID 16832、25124 已停止，31218 端口释放。v2215-v2218 已统一推送并发布 tag；唯一一次
  Node Evidence run `29794003784` 在 head `f5fa295a` 上通过 578 文件、1,758 测试、build 与全部安全
  smoke，job 总耗时 14 分 19 秒。批次机械验收完成。
