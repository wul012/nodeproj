...
| promotion Markdown 不再手写 1,681 行格式逻辑 | section/spec 数据 + 既有 report builder | 全部 28 artifact 固定时间 exact-byte oracle | v2228 core/handoff passed; v2229 release/deploy pending |
| 最长 section renderer 不再承担 200 行控制流 | section catalog + 小型领域 helper | fixed-time JSON/Markdown parity、最大函数 <=160 | v2230 pending |
| 最高复杂度 checks 不再使用单体布尔拼接 | 具名 predicate groups + strict-true assembly | READY/BLOCKED/缺失/错类型完整 parity | v2231 pending |
| 近限、复杂和命名债按绝对目标下降 | 从榜首继续做成组的真实 bounded-context 提取 | 九分 census + shrink-only baseline | v2232 及 successor pending |
...
## v2230：controlled shard 最长函数收敛

从最长函数榜首开始，至少处理 217、213、208、197、196 行的五个 section renderer。共性采用
declarative section catalog；特有分支留在短领域 helper。不得制造五个平移后的新大文件。版本完成条件是
单函数最大长度 <=170、长函数净减少至少 10，JSON/Markdown parity 与 route/archive 回归全绿。

## v2231：最高复杂度 checks 收敛
