...
| 要求 | 实现边界 | 机械证据 | 状态 |
| --- | --- | --- | --- |
| verification logic <=1,000 | 四个 formatter 迁入短 renderer/spec，使用既有 report builder | 迁移前冻结完整字节 hash；迁移后 exact match；family census | v2233 first pair passed, 1320→1146 |
| 近限文件 <=70 | packet、preflight、operator-window 移出 Markdown ownership | maintainability 三个 stale file key，物理行数核对 | pending |
| 长函数 <=70、最大 <=160 | 拆 163 行 loader 与至少一个相邻 long builder | focused profile parity + maintainability | pending |
| 复杂函数 <=170 | 从实时排名选择至少 23 个复杂函数，按完整 predicate/domain group 提取 | 每批 focused + complex stale keys；无 replacement growth | pending |
| name debt <=4,200 | 私有 mega type barrel 仅保留全仓真实 import 集合 | AST import/export 差集 test、typecheck、原 type barrel test、elegance | pending |
| 终局真实通过 | 十二项绝对阈值与底层 ready 同时为真 | `npm run elegance:nine` + full shard/coverage/build/smoke/CI | pending |
