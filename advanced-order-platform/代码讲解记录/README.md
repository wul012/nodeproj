...
inventory 包
 -> 库存模型、锁库存查询、预占库存、确认扣减、取消释放、退款回补和库存变更流水

order 包
 -> 订单模型、订单行、请求响应 DTO、状态历史、下单、支付、退款、取消、发货、完成、过期扫描和订单业务编排

payment 包
 -> 支付交易流水，记录订单支付成功和退款后的金额、状态、渠道和模拟交易号

outbox 包
 -> 记录订单创建、支付、退款、取消、过期、发货、完成事件，并用后台发布器标记 publishedAt
...
11-version-7-payment-transactions.md
 -> 理解第七版如何把支付动作沉淀成可查询、幂等的支付交易流水

12-version-8-refund-flow.md
 -> 理解第八版如何让 PAID 订单退款，并同步回补库存、记录退款流水和事件

13-version-9-inventory-movements.md
 -> 理解第九版如何把库存 available/reserved 的每次变化记录成可查询流水
```

## 一句话总览

一句话总结：这个项目的核心不是“能创建订单”这么简单，而是用 Spring Boot 把下单幂等、库存一致性、库存变更流水、支付/退款交易流水、支付/退款/取消/过期/发货/完成状态流转、状态历史、Outbox 发布标记和事件驱动雏形串成一个可以继续升级的后端系统。
