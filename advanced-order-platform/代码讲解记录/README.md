...
payment 包
 -> 支付交易流水，记录订单支付成功后的金额、状态、渠道和模拟交易号
...
一句话总结：这个项目的核心不是“能创建订单”这么简单，而是用 Spring Boot 把下单幂等、库存一致性、支付交易流水、支付/取消/过期/发货/完成状态流转、状态历史、Outbox 发布标记和事件驱动雏形串成一个可以继续升级的后端系统。
