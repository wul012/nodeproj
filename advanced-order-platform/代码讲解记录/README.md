...
order 包
 -> 订单模型、订单行、请求响应 DTO、下单、支付、取消、发货、完成、过期扫描和订单业务编排
...
一句话总结：这个项目的核心不是“能创建订单”这么简单，而是用 Spring Boot 把下单幂等、库存一致性、支付/取消/过期/发货/完成状态流转、Outbox 发布标记和事件驱动雏形串成一个可以继续升级的后端系统。
