...
Node Evidence CI 首轮只在 portable oracle 上失败；根因修复后的 run `29803696909` 全绿：579 个测试文件、
1,765 项测试，statements/branches/functions/lines coverage 为 95.67/87.95/98.52/95.64%，build、health、
零请求 metrics、`readyForReleaseEvidenceArchive=true` 且 `executionAllowed=false` 的 release smoke、服务停止和
post-cleanup 均通过。该复验对应提交 `882a551e`，没有第三次 CI。
