...
31214 端口已释放。首次远端 Node Evidence 在全部静态门通过后，只在新增的 preview
parity oracle 上失败：Linux JSON 比 Windows 多 38 字节，而两份 Markdown 完全一致。
本地 Linux Node 22 容器复现了 runner 的同一摘要，并把差异缩小到一个声明路径的两次
出现。测试 helper 现同时归一化运行目录和历史声明的仓库根，且只在路径边界替换；生产
输出、fixture 与四个冻结摘要均未修改。Windows 和干净 Linux 容器的 2 文件 3 项聚焦
测试都已通过，远端 Node Evidence 等待修复提交后的最终复跑。
