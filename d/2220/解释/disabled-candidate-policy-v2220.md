...
批次末的 Linux CI 进一步暴露出原 portable oracle 只规范了仓库根路径，却仍把历史文本证据的 CRLF/LF
差异编码进 `sizeBytes` 与 `digest`。修复只作用于测试副本：先按 LF 重算证据文本元数据，再规范路径；本节记录的
生产 raw 字节、blocker 顺序和业务输出均未改变。v2220 的 portable 摘要因此更新为真正与宿主无关的内容基线。
