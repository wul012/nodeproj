...
type CandidateDocumentIntakePacket = ControlledReadOnlyShardPreviewCandidateDocumentIntakePacket;

export function renderControlledReadOnlyShardPreviewCandidateDocumentIntakePacketMarkdown(
  packet: CandidateDocumentIntakePacket,
): string {
  return renderVerificationReportMarkdown({
    title: "Controlled read-only shard preview candidate document intake packet",
    meta: [
    ...
    sections: [
      {
        ...
        lines: renderVerificationSeparatedBlockLines(packet.slots, renderSlot),
        bodyLeadingBlankLine: packet.slots.length > 0,
      },
      {
        heading: "Intake Guards",
        lines: renderVerificationSeparatedBlockLines(packet.guards, renderGuard),
        bodyLeadingBlankLine: packet.guards.length > 0,
      },
    ],
...
function renderSlot(
  slot: CandidateDocumentIntakePacket["slots"][number],
): string[] {
  return [
...
function renderGuard(
  guard: CandidateDocumentIntakePacket["guards"][number],
): string[] {
  return [
