...
export function renderJavaMiniKvRouteCatalogCleanupFreshBaselineStabilityCloseoutArchiveVerificationMarkdown(
  profile: JavaMiniKvRouteCatalogCleanupFreshBaselineStabilityCloseoutArchiveVerificationProfile,
): string {
  return [
    "# Java / mini-kv route catalog cleanup fresh baseline stability closeout archive verification",
    "",
    `- Service: ${profile.service}`,
    `- Generated at: ${profile.generatedAt}`,
    `- Profile version: ${profile.profileVersion}`,
    `- Archive verification state: ${profile.archiveVerificationState}`,
    `- Ready: ${profile.readyForRouteCatalogCleanupFreshBaselineStabilityCloseoutArchiveVerification}`,
