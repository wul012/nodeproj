...
export function renderJavaMiniKvRouteCatalogCleanupExpandedStabilityCloseoutArchiveVerificationMarkdown(
  profile: JavaMiniKvRouteCatalogCleanupExpandedStabilityCloseoutArchiveVerification,
): string {
  return [
    "# Java / mini-kv route catalog cleanup expanded stability closeout archive verification",
    "",
    `- Service: ${profile.service}`,
    `- Generated at: ${profile.generatedAt}`,
    `- Profile version: ${profile.profileVersion}`,
    `- Archive verification state: ${profile.archiveVerificationState}`,
    `- Ready: ${profile.readyForRouteCatalogCleanupExpandedStabilityCloseoutArchiveVerification}`,
