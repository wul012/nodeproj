...
export function collectRecommendations(
  ...
  const reviewScope = formatReadOnlyReviewScope(sourceMatrixConsumptionPlan);
  return [{
    code: ready ? "CONSUME_SOURCE_MATRIX_PLAN_READ_ONLY" : "REPAIR_SOURCE_MATRIX_CONSUMPTION_PLAN",
    severity: "recommendation",
    source: "next-plan",
    message: ready
      ? `Consume the ${sourceMatrixConsumptionPlan.planStepRecordCount} source matrix plan step records (${stepRecordSummary}) while routing remains disabled; safety ${safetySummary}; risk ${riskSummary}; promotionHold ${promotionHold}; reviewScope ${reviewScope}.`
      : `Repair the source matrix consumption plan before consumption; blocked reasons: ${blockedReasons}; safety ${safetySummary}; risk ${riskSummary}; promotionHold ${promotionHold}; reviewScope ${reviewScope}.`,
  }];
}
...
export function createNextActions(
  ...
  if (ready) {
    return [
      ...
      `Limit sourceMatrixConsumptionPlan.readOnlyReviewScope (${formatReadOnlyReviewScope(sourceMatrixConsumptionPlan)}) to read-only operations.`,
