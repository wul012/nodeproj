...
export function renderControlledReadOnlyShardPreviewLiveReadOnlyWindowOperatorEvidenceValueSupplySignedApprovalCaptureArtifactDraftTextPackageComparedEvidenceCandidateMarkdown(
  ...
  return renderVerificationReportMarkdown({
    title: "Controlled read-only shard preview live read-only window operator evidence value supply signed approval capture artifact draft text package compared evidence candidate",
    meta: [
    ...
    sections: [
      {
        heading: "Candidate Sections",
        lines: renderVerificationSeparatedBlockLines(candidate.sections, renderSection),
        bodyLeadingBlankLine: candidate.sections.length > 0,
      },
      {
        heading: "Candidate Blockers",
        lines: renderVerificationSeparatedBlockLines(candidate.blockers, renderBlocker),
        bodyLeadingBlankLine: candidate.blockers.length > 0,
      },
    ],
  });
}
...
function renderSection(
  section: ControlledReadOnlyShardPreviewLiveReadOnlyWindowOperatorEvidenceValueSupplySignedApprovalCaptureArtifactDraftTextPackageComparedEvidenceCandidate["sections"][number],
): string[] {
  return [
...
function renderBlocker(
  blocker: ControlledReadOnlyShardPreviewLiveReadOnlyWindowOperatorEvidenceValueSupplySignedApprovalCaptureArtifactDraftTextPackageComparedEvidenceCandidate["blockers"][number],
): string[] {
  return [
