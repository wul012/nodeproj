...
import {
  ACCEPTED_MINI_KV_EXECUTE_WITH_WAL_REFERENCES,
  ENDPOINTS,
  JAVA_V89_CONTEXT_HEADER_FIELD_SOURCE,
  JAVA_V89_RUNBOOK,
  JAVA_V89_WALKTHROUGH,
  MINI_KV_RUNTIME_SMOKE,
  MINI_KV_V98_RUNBOOK,
  MINI_KV_V98_WALKTHROUGH,
  MINI_KV_VERIFICATION_MANIFEST,
  NODE_V232_GUARD_SOURCE,
  SANDBOX_GUARD_PROFILE_PATHS,
  SHA256_HEX,
  booleanField,
  readMiniKvRuntimeSmokeEvidence,
  readMiniKvVerificationManifest,
  recordField,
  stringArrayField,
  stringField,
  type MiniKvRuntimeSmokeEvidence,
  type MiniKvV98ExecuteWithWalReference,
  type MiniKvVerificationManifest,
} from "../evidence/managedAuditManualSandboxConnectionRehearsalPacketReviewEvidence.js";
...
function createMiniKvV98Reference(
  ...
  return {
    ...reference,
    readyForNodeV233RehearsalPacketReview: reference.evidencePresent
      && acceptedMiniKvExecuteWithWalReference(reference)
...
function acceptedMiniKvExecuteWithWalReference(input: {
  projectVersion: string;
  releaseVersion: string;
  consumerHint: string;
}): boolean {
  return ACCEPTED_MINI_KV_EXECUTE_WITH_WAL_REFERENCES.some((reference) =>
